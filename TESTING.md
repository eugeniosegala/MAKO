...
Every spatial-scaling change needs the proportionate real-Vulkan matrix in [Spatial scaling architecture](engine/docs/SCALING.md). At minimum, exercise a fresh process with Scaling Engine provisioned at the Native Resolution default and Frame Generation Off while proving that LSFG interop/backend/private resources are retained, no generated work is submitted, and only the model-free linear spatial baseline runs; turn Frame Generation On and Off live in that same process; exercise MAKO Scaler plus LS1 Quality and Performance independently and with Fixed and Adaptive Frame Generation; and verify missing DLL/translator/backend startup fails closed without blocking Native Resolution or MAKO Scaler. Cover fixed Gamescope with HDR exposure disabled, variable desktop surfaces including a compositor-echoed presentation extent, Vulkan-maximum clamping, device-memory admission and rejection at 1080p, 1440p, 4K, ultrawide, 5K and 8K presentation sizes, a 1080p→4K→1440p chain that retains its proven 4K presentation envelope, and non-widescreen source preservation, standard and high-precision SDR for every method including LS1's RGBA8 model-boundary conversion, every LS1 sharpness variant, ordinary queue 0 from an application-created graphics-and-compute family that presents to the surface, supported and rejected swapchain shapes, and natural swapchain recreation. A process started with Scaling Engine on must switch privately between Native Resolution and every scaler without recreating the game swapchain; sharpness must use the same private path, Scale Factor must wait for the next natural resolution boundary, rapid numeric edits must debounce and coalesce, and Flow Scale/Lighter FG Model/capacity changes must remain deferred until a compatible natural boundary. Managed Gamescope tests must reject any synthetic out-of-date request for those changes. An explicitly supported non-Gamescope compatibility lane may separately prove one maintenance1-fenced one-shot request without being treated as Gamescope evidence. Also cover replacement-context Flow/model/capacity evidence, Fixed above-half-refresh admission, injected lower-present stalls and escalating recovery, and fail-closed HDR/unsupported-format behavior. Resolution-churn coverage must prove one deferred and one completed lower retirement per game-requested recreation, completion only after a later replacement present plus the compositor grace and every associated MAKO fence, no duplicate lower destruction, no synchronization-validation error, and continued presentation. Scaling Engine itself remains process-static because its WSI chain is fixed at instance creation; only Frame Generation, model selection, sharpness, and other documented live controls change inside a provisioned process. Flow Scale, model, and capacity edits without retained FG resources must remain deferred. An echoed variable extent must stop scaling rather than be multiplied again. A present batch containing multiple swapchains and any active scaler must return before consuming application wait semaphores. Combined frame-generation runs still require separate FP32 and FP16 evidence. Record requested/active method plus source and presentation dimensions, present-retirement mode and deferred/completed records, device-local heap and presentation-pixel budget from Renderer policy logs, preserve Vulkan-loader evidence that the intended development layer was active, and compare image quality and GPU cost against a native-resolution reference.

MAKO Gym's 74-case procedural visual matrix measures LSFG, the production spatial scalers, and their real exported-image/timeline-semaphore handoff against vector-rendered references. It includes FP32/FP16, every spatial method, every scene, near-endpoint interpolation, near-native scaling, and representative Flow Scale/model/factor/sharpness interactions. A separate nine-case three-run sentinel requires byte-identical output across independent initialization, an 18-row repeated LSFG benchmark enforces practical 2× through 5× throughput and sample-variance budgets through 5120×2160, 36 exact-resolution rows score spatial pixels and measure complete production graphs with Vulkan timestamps, 14 paired live rows enforce present/frame p95/p99 plus CPU/RSS and exact MAKO-owned Vulkan-allocation budgets, and eight canonical paths run under Khronos synchronization validation. These lanes still do not measure subjective game quality, input-to-photon latency, power, or scanout cost. Likewise, a successful `vulkaninfo` or one-frame `vkcube` smoke test proves only the loader or basic presentation boundary. A release that changes spatial reconstruction must retain visual, deterministic, performance, synchronization, lifecycle, and relevant game/runtime evidence rather than treating any one gate as a substitute.
...
| Game-thread present/frame duration, Renderer process CPU, peak RSS, exact MAKO-owned Vulkan allocations, or reports that MAKO feels heavy | Runtime-overhead suite at `--tier fast`, widened by resolution or tier when a shared presentation/backend owner changed. |
...
| Release candidate | The required SteamOS workflow runs all 48 feature, 74 quality, 18 LSFG-performance, 36 spatial-performance, 14 runtime-overhead, eight synchronization-validation, nine repeatability-sentinel, 31 recovery, twelve native Gamescope E2E, twelve Proton E2E, and ten Proton compatibility sentinels across at least four runtime families against the exact source/package boundary. |
...
MAKO Gym's current 48-case manifest covers Scaling Engine absent and provisioned, the active model-free Native Resolution baseline, every MAKO and LS1 scaling model/variant, intentional fallback, Fixed, Adaptive, Steady/Smooth, Dynamic Cadence Recovery configuration, Ultra Performance, FP32/FP16 configuration, Flow Scale, Lighter FG Model, dedicated FP32 Quality and Performance 5× capacity, and representative scaling-plus-frame-generation combinations. Its steady finite-cube rows prove configuration and construction, not a recovery transition. Its own portable gate validates the declarative inventory and assertion contract without a GPU or DLL. The complete lane table, duration classes, pass criteria, outputs, and limitations are authoritative in `MAKO-Gym/docs/VULKAN-FEATURE-MATRIX.md`.
...
This separate twelve-row matrix automatically constructs controlled paths from the installed 64-bit Gamescope WSI and both exact MAKO package manifests. It proves the complete frame-generation-role → WSI → spatial-scaling-role order, scaling-disabled top-only isolation, missing-dependency fail-closed behavior, Fixed and Adaptive generated delivery, the active Native Resolution baseline, variable 640×360-to-960×540 scaling, every private live scaler method without game WSI recreation, live FG Off/On without an FG-only recreation, deferred/completed lower retirement, a natural recreation, a 1080p→4K→1440p retained presentation envelope, an 89→100 FPS boundary across that real resolution replacement, noisy 82/106 FPS excursions followed by sustained 82 FPS generation recovery, one compound single-process journey combining every scaler, FG Off/On, and the high-resolution transition, transport health, and clean shutdown. It is slower and environment-specific, so normal edits retain unit and integration coverage and run focused E2E rows only when the affected boundary warrants them; the complete E2E matrix is required before release. Its authoritative contract is in `MAKO-Gym/docs/GAMESCOPE-END-TO-END.md`.
...
This separate twelve-row matrix locally builds a source-only Windows workload and runs motion-boundary, traffic, crowd, camera-motion, HUD/disocclusion/particle, and mixed-stress scenes through each of D3D11/DXVK and D3D12/VKD3D-Proton inside the selected Proton version's matching Steam Linux Runtime Pressure Vessel. Each translation retains Fixed and Adaptive generation plus the active Native Resolution baseline, MAKO Scaler, LS1, and LS1 Performance. A pass requires in-container import of all three role manifests, exact translation-engine proof, the real frame-generation-role → Gamescope WSI → spatial-scaling-role chain, both MAKO role identities, the requested scaler, at least three positive generated-frame windows, all 360 bounded source frames, no unhealthy recovery/delivery record, role-balanced retirement, and clean process exit. VKD3D-Proton's deterministic destroy-before-create startup cycle must produce exactly one upper-role retained-lower `oldSwapchain` handoff, classify the new context as a replacement, and complete without a native-window-in-use retry or swapchain-creation failure. The runner owns a disposable Proton prefix and hard deadline; generated PE/DLL/cache data stays ignored and is never package content. Run focused rows while iterating and all twelve only before release or after shared translation/WSI changes. Its authoritative contract is in `MAKO-Gym/docs/PROTON-END-TO-END.md`.
...
2. runs 18 repeated LSFG performance workloads, including explicit 5× capacity, 36 exact-resolution pixel-qualified timestamp-query spatial-performance workloads, eight synchronization-validation paths, and nine three-run deterministic-output sentinels with the same source-built CLI;
...
7. passes that exact extracted Renderer launcher to MAKO Gym and requires all 48 native-Vulkan feature scenarios to pass;
8. builds Gym's test-only native-Vulkan workload and requires all 31 scripted recovery, cadence, live-transition, target-rate, and lifecycle scenarios plus 14 paired idle/active runtime-overhead rows to pass against the same extracted launcher;
9. requires all twelve native Gamescope E2E rows to load the host's real WSI between the exact packaged MAKO frame-generation and spatial-scaling roles, prove variable scaling, private live FG/scaler transitions, retained high-resolution presentation allocation, the 89→100 FPS replacement boundary, noisy high-base Adaptive recovery, the compound single-process transition journey, natural recreation, top-only isolation, and missing-dependency fail-closed behavior;
10. locally builds Gym's source-only Windows workload and requires all twelve D3D11/DXVK and D3D12/VKD3D-Proton scene rows to prove the translation engine, production split chain, scaling, generated delivery, role-balanced retirement, bounded completion, and clean shutdown;
