...
This separate eleven-row matrix automatically constructs a controlled chain from the installed 64-bit Gamescope WSI and exact MAKO package manifests. It proves loader membership and order, MAKO's allowed WSI presentation policy, Fixed and Adaptive generated delivery, the active Native Resolution baseline, variable 640×360-to-960×540 scaling, every private live scaler method without game WSI recreation, live FG Off/On without an FG-only recreation, deferred/completed lower retirement, a natural recreation, a 1080p→4K→1440p retained presentation envelope, an 89 FPS fractional-to-100 FPS near-target boundary across that real resolution replacement, one compound single-process journey combining every scaler, FG Off/On, and the high-resolution WSI transition, transport health, and the deliberately reversed fail-closed layer order. It is slower and environment-specific, so normal edits retain unit and integration coverage and run focused E2E rows only when the affected boundary warrants them; the complete E2E matrix is required before release. Its authoritative contract is in `MAKO-Gym/docs/GAMESCOPE-END-TO-END.md`.
...
9. requires all eleven native Gamescope E2E rows to load the host's real WSI layer above that exact packaged MAKO layer, prove variable scaling, private live FG/scaler transitions, retained high-resolution presentation allocation, the 89→100 FPS boundary across a real resolution replacement, the compound single-process transition journey, natural recreation, and the reversed-order fail-closed path;
