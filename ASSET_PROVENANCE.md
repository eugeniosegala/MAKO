...
| --- | --- | --- |
| `assets/inferno.png` | Maintainer-directed MAKO 4.0 release illustration generated with the built-in ImageGen tool on 23 September 2026: a mako surging through a volcanic sea between lava-lit Renaissance cities and ships. Existing MAKO banners informed the Renaissance pixel-art style; no source image was supplied to the initial generation call. A subsequent ImageGen edit used that generated banner to give the shark a calmer, more controlled pose at the maintainer's request. The 2172×724 RGB PNG is the direct edited output. Both prompts are retained locally under ignored `engine/out/inferno-artwork/` as `prompt.txt` and `controlled-shark-prompt.txt`; no third-party source artwork is intentionally incorporated. | Distributed under GPL-3.0-or-later to the extent copyright or other licensable rights exist and are controlled by the MAKO contributors. |
| `assets/neptune-fury.png` | Maintainer-directed MAKO 3.3 release illustration generated with the built-in ImageGen tool on 15 September 2026: Neptune commanding a storm beside a colossal mako and Renaissance fleet. Existing MAKO banners informed the Renaissance pixel-art style; the initial generation used no source image. A subsequent ImageGen edit used that generated banner to enlarge the shark at the maintainer's request. The edited output was resized from 2172×724 to 1536×512 using FFmpeg area scaling, quantized to a 64-colour PNG without dithering, and compressed at level 9 for release-page use (378,465 bytes). Both prompts are retained locally under ignored `engine/out/neptune-fury-artwork/` as `prompt.txt` and `larger-shark-prompt.txt`; no third-party source artwork is intentionally incorporated. | Distributed under GPL-3.0-or-later to the extent copyright or other licensable rights exist and are controlled by the MAKO contributors. |
| `assets/the-captain.png` | Maintainer-directed MAKO 3.2 release illustration generated with the built-in ImageGen tool on 6 September 2026. Existing MAKO banners informed the Renaissance pixel-art style; no source image was supplied to the generation call. Resized from 2172×724 to 1536×512 with `sips`, quantized to a 256-colour PNG with `pngquant`, and compressed with `pngcrush` for release-page use. No third-party source artwork is intentionally incorporated. | Distributed under GPL-3.0-or-later to the extent copyright or other licensable rights exist and are controlled by the MAKO contributors. |
