...
Use `TESTING.md` to select validation in proportion to the boundary. Renderer C++ changes need portable/full CTest; scheduling and presentation-policy work also needs the sanitizer path and applicable Adaptive matrix. Presentation, synchronization, device-feature, format, shader, scaling, or frame-quality changes need real Vulkan evidence through MAKO Gym. AMD paths require both FP32 and FP16 evidence. Decky backend changes need Python tests; frontend changes need typechecking, focused Vitest coverage, and a production bundle. Website changes need both builds in `website/README.md`. Packaging, installation, wrapper, architecture, or pin changes need package verification because unit tests do not prove layout or loader activation.
