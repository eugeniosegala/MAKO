...
| Private real-hardware QA | Licensed-model Vulkan/Gamescope feature, procedural-quality, and scripted recovery inventories, orchestration, assertions, and local evidence | Sibling `MAKO-Gym` checkout; MAKO owns only `engine/scripts/run-mako-gym.sh` and its portable bridge contract |
...
- AMD behavior must be checked in both FP32 and FP16 modes. The deterministic procedural scene catalog, spatial/combined reference generation, scoring implementation, and three real-GPU CLI paths live in `engine/mako-common/src/quality/`, `engine/mako-common/tests/`, and `engine/mako-cli/src/tools/quality.cpp`; MAKO-Gym owns the 66-case licensed hardware inventory, orchestration, assertions, PPM validation, and artifacts.
