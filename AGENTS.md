...
| Private real-hardware QA | Licensed-model Vulkan/Gamescope scenario inventory, orchestration, assertions, and local evidence | Sibling `MAKO-Gym` checkout; MAKO owns only `engine/scripts/run-mako-gym.sh` and its portable bridge contract |
...
| Run the native Vulkan feature matrix | `TESTING.md`, `engine/docs/SCALING.md`, `engine/docs/ADAPTIVE-VALIDATION.md`, sibling `MAKO-Gym/docs/` | `engine/scripts/run-mako-gym.sh`, sibling `MAKO-Gym/scripts/run-vulkan-feature-matrix.sh` |
...
- AMD behavior must be checked in both FP32 and FP16 modes. The deterministic scene and scoring implementation live in `engine/mako-common/src/quality/`, `engine/mako-common/tests/`, and `engine/mako-cli/src/tools/quality.cpp`; the licensed hardware orchestration and artifacts live in MAKO-Gym.
