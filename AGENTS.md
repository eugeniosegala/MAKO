...
| Run native Vulkan feature or recovery matrices | `TESTING.md`, `engine/docs/SCALING.md`, `engine/docs/ADAPTIVE-VALIDATION.md`, sibling `MAKO-Gym/docs/` | `engine/scripts/run-mako-gym.sh`, sibling `MAKO-Gym/scripts/run-vulkan-feature-matrix.sh`, sibling `MAKO-Gym/scripts/run-runtime-recovery-matrix.sh` |
...
- Real Vulkan feature scenarios are owned by MAKO Gym. Local MAKO validation may skip clearly when the sibling is absent; the SteamOS release gate must pass `--require`, validate the cross-repository contract version, run the exact packaged Renderer, and retain Gym's logs and summary.
