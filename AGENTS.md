...
| Run native Vulkan feature, recovery, compositor, or Proton translation end-to-end matrices | `TESTING.md`, `engine/docs/SCALING.md`, `engine/docs/ADAPTIVE-VALIDATION.md`, sibling `MAKO-Gym/docs/` | `engine/scripts/run-mako-gym.sh`, sibling `MAKO-Gym/scripts/run-vulkan-feature-matrix.sh`, sibling `MAKO-Gym/scripts/run-runtime-recovery-matrix.sh`, sibling `MAKO-Gym/scripts/run-gamescope-end-to-end.sh`, sibling `MAKO-Gym/scripts/run-proton-end-to-end.sh` |
...
- Renderer C++ changes need the portable/full CTest coverage described in `TESTING.md`. Presentation, synchronization, device-feature, image-format, shader-selection, spatial-reconstruction, or frame-quality changes also need real Vulkan evidence.
- Real Vulkan feature, Gamescope end-to-end, and Proton translation scenarios are owned by MAKO Gym. Local MAKO validation may skip clearly when the sibling is absent; the SteamOS release gate must pass `--require`, validate the cross-repository contract version, run the exact packaged Renderer, and retain Gym's logs and summary.
...
- AMD behavior must be checked in both FP32 and FP16 modes. The deterministic procedural scene catalog, spatial/combined reference generation, scoring implementation, and three real-GPU CLI paths live in `engine/mako-common/src/quality/`, `engine/mako-common/tests/`, and `engine/mako-cli/src/tools/quality.cpp`; MAKO Gym owns the 74-case licensed hardware inventory, LSFG/spatial/runtime performance budgets, deterministic-output sentinels, synchronization validation, orchestration, assertions, PPM validation, and artifacts.
...
- Website changes need both the Vinext production build and the GitHub Pages static build from `website/README.md`. Direct release-asset links must be verified against the current published component releases before changing the displayed version or filenames.
...
Completed comparative game sessions may be archived with `scripts/capture-trace.sh` into a sibling private `MAKO-Traces` checkout. Read `TRACES.md` before changing or running the extractor. The public MAKO repository owns extraction, component containment, sanitization, metadata production, the initial controlled-trace checksum manifest, and the producer contract test. MAKO Traces owns stored evidence, executable schemas, the canonical checksum contract and verification, guarded initialization and refresh, append-only history, and archive validation. Controlled-trace contract changes update both repositories together, while user-report-only contracts remain private-repository work. Normal MAKO builds, tests, packaging, installation, and releases must not require access to the private repository. Never copy licensed inputs or unrelated logs into a trace.

The sibling private `MAKO-Gym` checkout is the authority for MAKO Gym's real-hardware scenario manifests, orchestration, assertions, and local run evidence. MAKO owns `engine/scripts/run-mako-gym.sh`, the expected contract-version file, its synthetic bridge test, and the fail-closed SteamOS workflow integration. Ordinary builds and portable tests must remain usable without Gym; release hardware validation requires a clean compatible Gym checkout and records its commit. Cross-repository interface changes update both repositories together. Never copy Gym's scenario inventory, licensed inputs, or generated evidence into MAKO.
...
- `engine/mako-render/src/shaders/spatial_scaling_spirv.hpp` and `spatial_scaling_spirv.hashes` are generated from the adjacent spatial compute shader by `engine/scripts/generate-spatial-scaling-spirv.py`. The portable CTest suite runs its read-only `--check` mode against the source plus both SDR embedded payloads; regeneration requires `glslangValidator`. Do not edit generated arrays or hashes independently.
