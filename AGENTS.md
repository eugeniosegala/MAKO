...
Renderer HDR architecture is owned by `engine/docs/HDR-PIPELINE.md`; implicit
layer and Gamescope presentation ownership is owned by
`engine/docs/WSI-ISOLATION.md`. Read both before changing either boundary.
...
- Treat that schema as the Decky profile allowlist. Unknown keys are inert on
  read and removed by the next canonical write; wrapper keys cannot become
  environment exports unless the current schema and generator support them.
  Preserve changed semantics through an explicit, tested migration instead of
  retaining arbitrary legacy fields or reusing a retired name.
