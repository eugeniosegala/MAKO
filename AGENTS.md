...
The private real-hardware QA companion is **MAKO-Gym**. It is a sibling development repository, not a runtime component or package name.
...
| Private real-hardware QA | Licensed-model Vulkan/Gamescope feature, procedural-quality, repeatability, performance, and scripted recovery inventories, orchestration, assertions, and local evidence | Sibling `MAKO-Gym` checkout; MAKO owns only `engine/scripts/run-mako-gym.sh` and its portable bridge contract |
