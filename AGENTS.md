...
- Real Vulkan feature, Gamescope end-to-end, Proton translation, and multi-runtime Proton compatibility scenarios are owned by MAKO Gym. Local MAKO validation may skip clearly when the sibling is absent; the SteamOS release gate must pass `--require`, validate the cross-repository contract version, run the exact packaged Renderer, and retain Gym's logs and summary.
