...
The SteamOS release gate must require a compatible MAKO Gym checkout, run the exact packaged Renderer, and retain its logs and summaries. Armada remains the fail-closed boundary in `plugin/docs/ARMADA.md`: do not add an opaque native binary, second host detector, or global FEX mutation; enabling it requires source-built AArch64 packaging and real-hardware evidence.
