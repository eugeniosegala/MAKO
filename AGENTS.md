...
| Build host or complete local Renderer artifacts | `engine/docs/BUILDING-FROM-SOURCE.md` | `engine/scripts/package-local.sh`, `engine/scripts/package-local-release.sh` |
...
| Check a complete local release candidate | `TESTING.md`, `HOW_TO_RELEASE.md` | `scripts/check-release-candidate.sh` |
| Validate real SteamOS hardware | `TESTING.md` | `engine/scripts/run-mako-gym.sh`; sibling Gym owns hardware runners |
...
Use `TESTING.md` to choose proportionate evidence. Renderer C++ changes need portable/full CTest; scheduling and presentation work also needs sanitizers and the applicable Adaptive matrix. Choose targeted MAKO Gym evidence for Vulkan, scaling, synchronization, shader, quality, or resource-lifetime boundaries when the change needs hardware qualification, including FP32 and FP16 for AMD paths; report untested hardware rows explicitly. Decky changes need their backend or frontend gates, website changes need both documented builds, and package-boundary changes need package verification.

Before publication, check one complete portable Decky ZIP built from the clean pushed candidate with `scripts/check-release-candidate.sh`; reuse that ZIP rather than rebuilding it in CI. MAKO Gym suites are targeted, optional hardware evidence chosen for the changed boundary, and untested rows must be reported. Publication rebuilds after release-owned metadata commits, so the published-asset install check is separate. Run every Gym suite only for a genuinely cross-cutting change or explicit maintainer request. Armada remains fail-closed under `plugin/docs/ARMADA.md`; enabling it requires source-built AArch64 packages and real-hardware evidence, not an opaque binary, second detector, or global FEX mutation.

When claiming complete Renderer hardware qualification, include the bounded MAKO Gym resource-constraint suite against the relevant packaged CLI and launcher. Its case/profile mapping remains owned by Gym.
...
- Published Renderer URLs, checksums, and pins in `plugin/package.json` are maintained by the release/pinning scripts. `engine/dist/arch/sync-release-pin.py` derives the Arch recipe's version and checksum from that verified pin; use `just check-arch-package` instead of editing those fields independently. Local-engine packaging writes local identity into the ZIP without changing the tracked release pin.
...
Publishing is a separate, explicitly authorized workflow. Follow `HOW_TO_RELEASE.md` from a clean `main` checkout, update the two component release-note files as the only manual release copy, check the complete local candidate ZIP, and let the scripts manage versions, pins, checksums, tags, assets, and README release links. Choose MAKO Gym hardware checks as needed for the changed boundary; no GitHub hardware runner is part of the release workflow. Never move a published tag or replace a published asset.
