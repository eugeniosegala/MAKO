...
| Per-game Flatpak launcher preparation | `FlatpakService._check_app_override_status()` and `set_app_override()`, classified by `shared_config.PER_GAME_WRAPPER_FLATPAK_APP_IDS` | Lutris could be prepared app-wide by older releases. Launcher preparation is incomplete while managed layer variables remain; preparing again clears those variables and preserves unrelated overrides. | Every supported installation has per-game launcher preparation or an equivalent checked migration. | Heroic and Lutris fixtures reject stale activation, prove idempotent re-preparation and retained unrelated settings, and surface cleanup failures; direct emulator preparation remains app-wide. |
