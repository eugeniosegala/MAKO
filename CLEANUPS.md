...
| Pre-Steady-default Adaptive profiles | `ConfigurationManager._config_from_profile()` | Profiles saved before `adaptive_auto_base_fps_cap` was persisted used Fractional Adaptive; applying the new Steady 2x default to a missing field would silently change their pacing. | The minimum supported direct-upgrade baseline only includes releases that persist `adaptive_auto_base_fps_cap`. | A version 2 profile missing the field continues to parse as Fractional while a genuinely new profile defaults to Steady 2x. |
