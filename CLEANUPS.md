...
| Ubisoft launcher executable exclusion | Shared `engine/mako-common/launcher_exclusions.json`, generated by `scripts/generate-launcher-exclusions.py` for Renderer activation and Decky capture | Launcher UI processes inherit game profile overrides and fallback activation; older captured profiles can also name them explicitly. Exact Ubisoft Connect launcher basenames stay inactive in the Renderer and are excluded from Decky capture, without removing the child game's environment. Removal gate: A positively identified game-only activation mechanism replaces inherited process-tree activation, or real launcher evidence establishes support for MAKO rendering in every excluded executable. | Configuration tests reject override, fallback, explicit alias, and environment-only activation in each launcher while preserving games, unrelated basenames, and child activation; capture tests cover truncated process names; the cross-component contract validates the shared registry and checks both generated lists for freshness without rewriting them. |
| Pre-`VK_EXT_present_timing` build headers | `engine/mako-render/src/pnext_chain.hpp` | Supported build SDKs can predate an application/driver's `VkPresentTimingsInfoEXT`; filtering a later present-mode or damage-region node must still preserve the complete timing prefix. The local layout copies only the outer node and keeps its timing payload opaque. Removal gate: Every supported host and Flatpak build SDK exposes `VK_EXT_present_timing`; replace the local layout and numeric structure identifier with the SDK declarations while retaining runtime support. | `pnext-chain` regressions pass with old and new headers; new SDKs statically verify the structure identifier, size, alignment, and every field offset. |
| Per-game Flatpak launcher preparation | `FlatpakService._check_app_override_status()` and `set_app_override()`, classified by `shared_config.PER_GAME_WRAPPER_FLATPAK_APP_IDS` | Lutris could be prepared app-wide by older releases. Launcher preparation is incomplete while managed layer variables remain; preparing again clears those variables and preserves unrelated overrides. Flatpak's `unset-environment` takes precedence over its empty compatibility entries when checking remaining activation. Removal gate: Every supported installation has per-game launcher preparation or an equivalent checked migration. | Heroic and Lutris fixtures reject stale activation, prove idempotent re-preparation with Flatpak's serialized unset entries and retained unrelated settings, and surface cleanup failures; direct emulator preparation remains app-wide and rejects explicitly unset activation. |
