...
| Unreleased scaling Decky listing name | `plugin/scripts/deploy-dev.sh` and `plugin/scripts/deploy-validated-package.py` | Some 3.0 developer packages declared `MAKO - Scaling & Frame Generation` before the published listing identity was restored. Local deployment must accept that preview identity so the shared `Mako/` installation can return to `MAKO - Frame Generation`. | The affected pre-release developer packages are outside the supported local upgrade path. | Package-deployment fixture restores the established identity from a preview manifest in the `Mako/` directory; path-package contract proves both deployers retain the preview alias. |
