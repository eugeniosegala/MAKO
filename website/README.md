...
Run `npm install`, then `npm run dev` for the local preview. Run `npm run build` to validate the Vinext production build and `npm run build:pages` to create the GitHub Pages artifact under `dist-pages/`. Both builds validate the website's GPL declaration, deployed third-party notices, and visible notices link through `npm run check:legal`.
