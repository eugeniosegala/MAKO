...
3. records the exact Renderer tag, source commit, URLs, and checksums in `plugin/package.json`, synchronizes the Arch `mako-renderer-bin` recipe to that immutable host archive, verifies both pins, and builds the synchronized recipe against the exact local archive without installing or publishing the resulting package;
