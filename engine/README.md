...
4. Select Fixed or Adaptive Frame Generation and/or **Scaling (Restart)**. For scaling, choose the method and factor and lower the game's resolution as described in [desktop scaling and resolution](docs/CONFIGURATION.md#desktop-scaling-and-resolution). On a supported Gamescope HDR display, **HDR Output** also provides a per-profile **HDR Brightness Boost** and 203–1000-nit target while Steam HDR is on and in-game HDR is off.
