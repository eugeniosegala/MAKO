...
2. Under **Profile Matching**, add the game's Linux binary, Windows executable, or process name to **Matched Processes**. The selected profile's current matches remain visible in the main window. Start with fixed **2x** frame generation, or enable Scaling before launch so MAKO Decky can provision its guarded Gamescope presentation path and then select the intended lower rendering resolution in the game. Fixed/Adaptive mode, target, and multiplier changes apply live; capacity growth, provisioned scaling controls, Flow Scale, and Lighter FG Model use one game-owned swapchain recreation after the edits settle when needed, so a brief flicker is expected. Restart after the first off-to-on Scaling change if the process started on the isolated path, if the title ignores the recreation signal, or before enabling Frame Generation when the current process started scaling-only.
...
Every game, renderer, and display setup behaves differently. Compare scaling-only, Fixed Frame Generation, and Adaptive Frame Generation one setting at a time. Fullscreen is usually the best starting point for performance and frame pacing. Fixed/Adaptive mode, target, and multiplier changes apply live; capacity growth, scaling controls in a process launched with Scaling enabled, Flow Scale, and Lighter FG Model use one game-owned swapchain recreation when needed. Restart if Scaling was off when the isolated process started, if the title ignores the recreation signal, after major display, DLL, GPU, or process/device-policy changes, or before enabling Frame Generation in a process that started scaling-only.
