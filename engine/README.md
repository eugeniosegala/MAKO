...
CTest remains portable and validates the five-scene procedural image-quality catalog, spatial/combined references, masks, scoring policy, and corruption detection without a licensed DLL. Real AMD execution is owned by the private sibling MAKO-Gym checkout: its 66-case visual matrix covers LSFG FP32/FP16, every spatial method, and production scaling-to-LSFG handoffs, while its 20-row default native-Vulkan recovery matrix controls cadence rises, false probes, hitches, stalls, drops, old-swapchain replacement, near-target cadence, ordered 4× health, target-rate control, and live scaling lifecycle changes. Both are mandatory in the SteamOS release gate; two additional WSI order rows remain explicit compatibility evidence. Use `./scripts/run-mako-gym.sh --suite quality --cli <built-mako-cli>` or `./scripts/run-mako-gym.sh --suite recovery` from the MAKO root for the same local bridges.
