...
CTest remains portable and validates the five-scene procedural image-quality catalog, spatial/combined references, masks, scoring policy, and corruption detection without a licensed DLL. Real AMD execution is owned by the private sibling MAKO-Gym checkout: its 74-case visual matrix covers LSFG FP32/FP16, every spatial method, edge parameters, and production scaling-to-LSFG handoffs; nine three-run sentinels detect nondeterministic output; eight repeated workloads enforce practical LSFG throughput and variance budgets; nine timestamp-query workloads measure complete spatial graphs; eight canonical paths run under Khronos synchronization validation; and its 29-row default native-Vulkan recovery matrix controls cadence boundaries, repeated stalls, drops, single/chained old-swapchain replacement, near-target/high-base cadence, ordered 2×/3×/4× target control, and live scaling lifecycle/coalescing. Every default suite is mandatory in the SteamOS release gate; two additional WSI order rows remain explicit compatibility evidence. Use `./scripts/run-mako-gym.sh --suite <vulkan|quality|repeatability|performance|spatial-performance|sync-validation|recovery>` from the MAKO root for the local bridges.
