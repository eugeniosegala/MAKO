...
printf '%s\n' '#!/usr/bin/env bash' 'printf "pacing:%s\\n" "$@"' \
    > "$fake_gym/scripts/run-vrr-pacing-matrix.sh"
chmod +x "$fake_gym/scripts/run-vrr-pacing-matrix.sh"
...
printf '%s\n' '#!/usr/bin/env bash' 'printf "direct-desktop-e2e:%s\\n" "$@"' \
    > "$fake_gym/scripts/run-direct-desktop-end-to-end.sh"
chmod +x "$fake_gym/scripts/run-direct-desktop-end-to-end.sh"
...
printf '%s\n' '#!/usr/bin/env bash' 'printf "constraints:%s\\n" "$@"' \
    > "$fake_gym/scripts/run-resource-constraints.sh"
chmod +x "$fake_gym/scripts/run-resource-constraints.sh"
...
pacing_forwarded="$($bridge --gym-repo "$fake_gym" --suite pacing --filter '^vrr-')"
pacing_expected=$'pacing:--filter\npacing:^vrr-'
if [[ "$pacing_forwarded" != "$pacing_expected" ]]; then
    echo "Gym pacing arguments were not forwarded exactly." >&2
    exit 1
fi
...
direct_desktop_forwarded="$($bridge --gym-repo "$fake_gym" --suite direct-desktop-e2e --filter '^steam-desktop-dxvk-')"
direct_desktop_expected=$'direct-desktop-e2e:--filter\ndirect-desktop-e2e:^steam-desktop-dxvk-'
if [[ "$direct_desktop_forwarded" != "$direct_desktop_expected" ]]; then
    echo "Gym direct-desktop E2E arguments were not forwarded exactly." >&2
    exit 1
fi
...
constraints_forwarded="$($bridge --gym-repo "$fake_gym" --suite constraints --affected-suites recovery --require-complete)"
constraints_expected=$'constraints:--affected-suites\nconstraints:recovery\nconstraints:--require-complete'
if [[ "$constraints_forwarded" != "$constraints_expected" ]]; then
    echo "Gym mandatory constraint arguments were not forwarded exactly." >&2
    exit 1
fi

all_suites_forwarded="$($bridge --gym-repo "$fake_gym" --all-suites --validate)"
all_suites_expected=$'--validate\nquality:--validate\nrepeatability:--validate\nperformance:--validate\nspatial-performance:--validate\nruntime-overhead:--validate\nsync-validation:--validate\npacing:--validate\nrecovery:--validate\nexternal-recovery:--validate\ngamescope-e2e:--validate\ndirect-desktop-e2e:--validate\nsustained-health:--validate\nproton-e2e:--validate\nproton-compatibility:--validate\nconstraints:--validate'
if [[ "$all_suites_forwarded" != "$all_suites_expected" ]]; then
    echo "Gym all-suites validation did not invoke all sixteen runners exactly once." >&2
    exit 1
fi
