...
- **Smooth Cadence can converge Adaptive on Fixed-like 2x pacing:** On ordered Gamescope SDR when Target FPS matches display refresh, Fractional Adaptive may test a nearby constant 2x cadence and keep it only after output proves it can settle within 98–102% of target. A validated Steady 2x path can hand pacing to ordered FIFO instead of retaining a second software cap, while immediate rollback and long retry guards prevent an unsuitable game from receiving repeated pacing disturbances.
