...
- **Steam Desktop FPS counter:** Native and Proton Steam launches now preserve Steam's requested Vulkan overlay after MAKO's managed layers in Desktop Mode, while keeping Fossilize and unrelated implicit layers isolated. Gaming Mode and Flatpak presentation paths are unchanged.
...
- **Fractional Real Frame Priority:** Fractional Adaptive can now apply optional Low, Medium, High, or Very High real-frame priorities using stable 3:2, 2:1, 3:1, and 4:1 real/generated ratios. The default Automatic setting preserves the previous scheduler and manual-cap behavior. Explicit priorities use the existing live real-frame pacer, remain dormant outside Fractional Adaptive, and preserve the saved manual Base FPS Cap.
