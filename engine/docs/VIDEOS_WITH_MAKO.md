...
Frame Generation and LS1 scaling require a lawful, user-supplied installation of the **default public version** of Lossless Scaling from Steam and its `Lossless.dll`. MAKO can use beta branches, but they are not validated; the default public branch is recommended. The open MAKO Scaler does not require the DLL.
