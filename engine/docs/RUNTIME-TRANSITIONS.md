...
| Game-owned swapchain resources | Spatial-scaling shape and extent contract | A game-owned recreation; the managed Gamescope lower spatial role and compatible non-Gamescope maintenance1 contexts may request one after a retirement-fenced present, while unsupported paths wait for a natural recreation |
...
| Scaling factor | Game-owned recreation | Existing context retains its extent contract; after edits settle, the managed Gamescope lower spatial role or a compatible non-Gamescope maintenance1 context requests one recreation after a fenced present, while the upper Gamescope role and contexts without retirement proof wait for the next natural resolution/surface recreation |
...
- Do not initiate game-owned swapchain destruction or synthesize an out-of-date result for a private-resource edit. The sole requested-recreation exception is a coalesced Scale Factor extent change on an eligible lower spatial role after a successful maintenance1-fenced present, and it may signal out-of-date only once. Remove a game-destroyed context from live updates immediately, and defer its lower WSI destruction until the replacement-present, grace-period, and fence conditions are all satisfied; only explicit destruction of that swapchain's creating surface may complete fence-proven terminal retirement without a later present.
