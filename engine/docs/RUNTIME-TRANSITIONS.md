...
On the managed Gamescope split, a Scale Factor or Quality Supersampling extent change may request application-visible recreation only from the upper spatial-resource owner. A compatible non-Gamescope maintenance1 context may request recreation for private-resource growth that cannot fit the current WSI pool. Recovery requests use the same successful, retirement-protected lower-present boundary; MAKO never destroys the active game-owned swapchain itself. Repeated cadence stalls or low FPS alone do not authorize performance-driven recreation.

The surface owner gates Adaptive performance recovery on a confirmed Gamescope Steam-focus return and a compact pre-menu performance history. It compares returned output with the lower of the captured median and configured target, so a game that was already stable below target is judged against its own performance rather than an unattainable number. At most two watchdog actions are allowed during a 75-second return episode. Baseline evidence survives only a MAKO-requested same-policy replacement; unknown/stale focus, Off/ineligible modes, changed settings/geometry, or unrequested replacement invalidate it. Menu-open delivery is excluded from scheduler learning and deficit evidence. Returned output remains visible to the watchdog during warm-up, scheduler stabilization, and transport recovery, but those recovery samples cannot update the healthy performance history. Entering confirmed Steam UI focus also clears incomplete ordered-acquire guard, drain, probe, stabilization, and episode evidence across exact Fixed, Fixed + Dynamic Cadence Recovery, Fractional Adaptive, and Smooth/Steady Adaptive; focus return retains the per-context recreation budget but starts transport qualification from fresh gameplay presents. A live mode, multiplier, cap, target, Smooth Cadence, or Dynamic Cadence Recovery change likewise clears transient admission, acquire, lower-present, and pipeline-busy evidence before the new workload begins without rearming any per-context recreation one-shot. Repeated returned native presents of at least 250 ms retain transport-failure recreation in Fixed and Adaptive with 30-second spacing and bounded native stabilization, but the recreation request is permitted only inside the same event-recovery boundary. Direct generated-image starvation is likewise transport-owned and event-scoped: inside a confirmed menu-return window, three distinct native-drain episodes within 15 seconds or one continuously failing three-second episode may request one retirement-protected recreation after bounded probes fail. A live Fractional/Steady change instead arms a 60-second transition watch in which three native-drain episodes provide the same direct evidence. An acquire timeout during an experimental scheduler multiplier rejects that probe immediately only while one of those recovery windows is active. During ordinary gameplay, Adaptive and Fixed + Dynamic Cadence Recovery both retain the 3.3 timeout behavior and history boundary rather than converting a generic transport miss into multiplier-failure evidence; exact Fixed has no scheduler probe. Low FPS remains ineligible everywhere. Frame Generation-only watchdog recovery may request recreation immediately after qualification. A qualified combined spatial-scaling regression first resets generation policy and temporal history in place; only a deficit that persists for three seconds after that attempt may request one retirement-protected application-owned recreation, and create-time memory-constrained scaling remains ineligible. Backend/fence and acquire safety remain independent. [Adaptive validation](ADAPTIVE-VALIDATION.md) owns thresholds and negative cases.
...
A scaled null-old replacement presents its first acquired application image directly before private spatial work. On the isolated Gamescope scaling surface, every replacement also creates a fresh compositor protocol object and publishes the replacement's actual image count and format before its first present; the old compositor pacing lifetime is not reused. A known replacement with Frame Generation active then presents real frames for 250 ms and warms three new history frames inside scheduler stabilization. A replacement created while Frame Generation is Off does not arm that FG-only interval.
...
With diagnostics enabled, `runtime-transition-pending`, `runtime-transition-prepared`, `runtime-transition-failed`, `runtime-transition-applied`, and `runtime-transition-recreation-requested` identify the configuration boundary and result. `gamescope-focus`, `adaptive-recovery-recreation-watchdog`, `adaptive-recovery-recreation-requested`, and `adaptive-recovery-in-place-requested` distinguish observed focus, baseline-relative deficits, guarded recreation, and scaled in-place recovery. The `adaptive-recovery-*` operation names are retained diagnostic compatibility identifiers, while their `mode` field distinguishes the shared Fixed and Adaptive post-menu policy. Swapchain create, replacement, retirement, WSI-prime, backend-stabilization, and memory records provide lifecycle evidence.
