...
4. Toggling Ultra Performance is a process-restart transition, but compatible scheduler and scaler fields from the same write can still apply at their normal boundaries.
...
7. Scaling, Flow Scale, model, or generated-capacity edits arm one debounced recreation request after the lower WSI has consumed the application's wait semaphores; pacing waits for a natural recreation. Diagnostics report live application, recreation deferral/request, and process-restart deferral independently. One update may have all three outcomes.
...
| `frame_generation_enabled = true` | Live when retained private resources exist; otherwise process restart | Reuses reserved resources, resets admission/recovery state, and starts with fresh temporal history where required; a scaling-only process has no device interop to add during recreation |
...
| Spatial-scaling enable, method, factor, and sharpness | Requested recreation | Existing context retains its actual surface/pipeline state; a debounced one-shot out-of-date result asks the game to recreate |
...
If the process started with Frame Generation enabled, it retains idle private source images, generated images, synchronization objects, and the backend context after a live disable so Frame Generation can be enabled again without recreation. The native presentation branch bypasses those objects while Off. A process that starts with Frame Generation disabled does not negotiate its external interop or construct its private backend, so enabling it later requires restart. A stabilized HDR colour transition may still update retained private colour resources so a later live re-enable uses the correct encoding.

When an Ultra Performance startup profile explicitly sets Frame Generation Off, MAKO skips generation-specific application instance/device/swapchain modifications and omits the private generation context and images. If Frame Generation is configured On but temporarily paused by the refresh threshold, resources remain present because compositor refresh feedback can enable it live. Compatible profile changes remain observable; toggling the Ultra preset itself still waits for restart.
...
With presentation diagnostics enabled, `operation=runtime-transition-pending` records the individual scaling/model/capacity flags and distinguishes `action=signal-out-of-date-after-successful-present`, `action=wait-for-natural-swapchain-recreation`, and `action=wait-for-process-restart`. A mixed request may report live application and more than one pending boundary for one context and state revision.
