...
| Gamescope VRR and Allow Tearing feedback | Live | Explicit requested/capable/active VRR can change Steady Adaptive's higher-multiplier cap eligibility and enable full-batch FIFO for a validated higher rung. Fixed Smooth Cadence and qualified Steady Adaptive handoffs retain ordered FIFO pacing. Only affected pacing helpers reset; Allow Tearing is diagnostic only. |
...
The surface owner accepts recovery authority only from explicit Gamescope focus transitions, live generation-policy transitions, and direct Vulkan outcomes. Entering confirmed Steam UI focus clears incomplete ordered-acquire retry state across exact Fixed, Fixed + Dynamic Cadence Recovery, Fractional Adaptive, and Smooth/Steady Adaptive. A confirmed return or live mode, multiplier, cap, target, Smooth Cadence, or Dynamic Cadence Recovery change starts fresh history and arms one recreation permission. Policy edits received while confirmed Steam UI focus already suspends generation are last-value-wins: they do not repeatedly rebuild the scheduler or arm parallel permissions, and the final scheduler policy is constructed once when gameplay returns. The return event supplies the single transport permission. The first successful generated-image acquire consumes that permission without action; an actual `VK_TIMEOUT`, `VK_NOT_READY`, or exhausted configured acquire budget may instead consume it by requesting one retirement-protected recreation. There is no seconds-long event window, failure-count qualifier, output-FPS comparison, successful-call duration threshold, or surface cooldown. Replacement contexts initialize from the current Gamescope return sequence, preventing the same old event from rearming a recreation loop. Ordinary gameplay FPS, cadence loss, scaling cost, post-processing cost, target deficit, and slow successful presents cannot obtain transport-recovery authority or request recreation; exact Fixed never launches a cadence-collapse probe. Backend/fence and acquire safety remain independent. [Adaptive validation](ADAPTIVE-VALIDATION.md) owns the direct-outcome state machines and negative cases.
...
With diagnostics enabled, `runtime-transition-pending`, `runtime-transition-prepared`, `runtime-transition-failed`, `runtime-transition-applied`, and `runtime-transition-recreation-requested` identify the configuration boundary and result. `gamescope-focus` and `ordered-acquire-*` distinguish observed focus, explicit acquire failure, bounded retry, and transition-scoped recreation. Swapchain create, replacement, retirement, WSI-prime, backend-stabilization, and memory records provide lifecycle evidence. The diagnostics helper continues to recognize retired slow-acquire, lower-present, and performance-watchdog operation names when collecting logs from older installations.
