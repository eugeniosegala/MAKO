...
The surface owner gates Adaptive performance recovery on a confirmed Gamescope Steam-focus return and a compact pre-menu performance history. It compares returned output with the lower of the captured median and configured target, so a game that was already stable below target is judged against its own performance rather than an unattainable number. At most two watchdog actions are allowed during a 75-second return episode with 30-second surface spacing. Baseline evidence survives only a MAKO-requested same-policy replacement; unknown/stale focus, Off/ineligible modes, changed settings/geometry, or unrequested replacement invalidate it. Menu-open delivery is excluded from scheduler learning and deficit evidence. Separately, repeated returned native presents of at least 250 ms retain transport-failure recreation in Fixed and Adaptive with 30-second spacing and bounded native stabilization; confirmed Steam UI focus suppresses that timing-based escalation. Both automatic recreation paths are limited to Frame Generation-only contexts. A qualified combined spatial-scaling regression instead resets generation policy and temporal history in place while retaining the scaled swapchain, then waits for natural game-owned recreation if presentation remains degraded. Backend/fence and acquire safety remain independent. [Adaptive validation](ADAPTIVE-VALIDATION.md) owns thresholds and negative cases.
