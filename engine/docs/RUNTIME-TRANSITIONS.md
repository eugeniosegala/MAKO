...
The surface owner gates Adaptive performance recovery on a confirmed Gamescope Steam-focus return and a previously healthy target baseline. At most two watchdog requests are allowed during a 75-second return episode with 30-second surface spacing. Baseline evidence survives only a MAKO-requested same-policy replacement; unknown/stale focus, Off/ineligible modes, changed settings/geometry, or unrequested replacement invalidate it. Menu-open delivery is excluded from scheduler learning and deficit evidence. Separately, repeated returned native presents of at least 250 ms retain transport-failure recreation in Fixed and Adaptive with 30-second spacing and bounded native stabilization; confirmed Steam UI focus suppresses that timing-based escalation. Both automatic recreation paths are limited to Frame Generation-only contexts. Combined spatial-scaling contexts recover in place or wait for a natural game-owned recreation, avoiding a full scaled-context rebuild during possible device-memory pressure. Backend/fence and acquire safety remain independent. [Adaptive validation](ADAPTIVE-VALIDATION.md) owns thresholds and negative cases.
...
With diagnostics enabled, `runtime-transition-pending`, `runtime-transition-prepared`, `runtime-transition-failed`, `runtime-transition-applied`, and `runtime-transition-recreation-requested` identify the configuration boundary and result. `gamescope-focus`, `adaptive-recovery-recreation-watchdog`, and `adaptive-recovery-recreation-requested` distinguish observed focus, qualified deficits, and guarded requests. Swapchain create, replacement, retirement, WSI-prime, backend-stabilization, and memory records provide lifecycle evidence.
