...
| Generated-image drain | 250, 500, 1000, 2000, 5000, 15000, then 30000 ms after an acquire timeout, budget exhaustion, or configured cumulative deadline overrun | Pace bounded retries after the explicit transport budget is missed; native FPS and unrelated presentation duration have no authority |
...
2. **Acquire-budget failure:** `VK_TIMEOUT`, `VK_NOT_READY`, exhaustion of the configured cumulative application-present budget, or completing the batch after that cumulative wall-clock budget is reached starts native backoff. The budget is explicit; FPS, cadence, resolution, scaling, and shader cost do not derive or change it.
...
4. **Transition-scoped recreation:** a confirmed menu return or explicit live generation-policy change arms one permission. An acquire batch completed within its configured budget consumes it. An acquire-budget failure may consume it by requesting one retirement-protected game-owned recreation; there are no separate event windows, FPS qualifiers, or failure-count thresholds.
