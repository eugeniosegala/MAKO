...
| Gamescope VRR pacing ownership | Avoid treating FIFO backpressure as a periodic clock when Gamescope explicitly requests or reports adaptive scanout | Verified server-zero properties; missing initial support preserves prior behavior; live changes reset only pacing helpers | **Explicit compositor contract, not a performance heuristic.** It cannot select a multiplier, start recovery, or request recreation |
