...
| Lower-present stall quarantine | A single returned call must block for at least 250 ms; recovery lasts 2, 10, 30, then 60 seconds; consecutive count clears after 30 seconds healthy; two additional severe native stalls arm recreation with 30-second surface spacing only without active spatial scaling; one second without severe native stalls cancels it; unavailable rebuilds wait only until the later of the original deadline and 30 seconds from quarantine start | Keep generated delivery away from a repeatedly blocking lower path without an indefinite native-only latch; external throttling is not proof of a wedged FIFO, and combined scaling never forces a recovery rebuild |
...
| Adaptive post-menu watchdog | Confirmed focus round trip; median of eight allocation-free quarter-second samples spanning at least 1 second before opening; then 4 seconds below 75% of the lower of pre-menu output and target with at least 50% lower-present time | At most two guarded actions per 75-second episode; Frame Generation-only may request recreation with 30-second surface spacing, while combined scaling resets generation policy/history first and may request one non-memory-constrained recreation only if the deficit persists for 3 seconds after that attempt; unknown/stale focus or policy/geometry changes invalidate evidence |
...
MAKO uses a ladder rather than one global health timer. Confirmed Steam UI focus suspends generated work and scheduler observations while preserving real/scaled presentation. Return refreshes three history frames, retains the proven generated level, and starts a bounded recovery episode; an eligible confirmed return settles for 250 ms rather than the ordinary one second. Unknown focus, an unproven policy after a live reset, and unsupported transports retain conservative settling. The shorter path cannot advance an already-active stabilization deadline, and a second interruption restarts the full history warm-up in both Fixed and Adaptive modes. History completion and GPU/transport safety remain mandatory independently of the shorter timer; low FPS alone is never treated as a menu event. Focus changes require 250 ms agreement; samples expire after one second without monitor updates.
...
7. **Lower-present quarantine:** outside confirmed Steam UI focus, a returned lower present slower than the larger of 250 ms or four refresh periods quarantines generation for 2, 10, 30, then 60 seconds.
...
| Returned lower-present stall quarantine | Stop adding work after the driver/compositor has demonstrably stalled for at least 250 ms | Logging-independent timing, severe-stall floor, bounded durations, healthy cancellation, one-shot guarded recreation after repeated native stalls, and fresh history before retry | **Reasonable reactive protection.** It cannot prevent the first blocking call or distinguish external throttling from a wedged FIFO; a bounded retry is not proof of recovered performance |
