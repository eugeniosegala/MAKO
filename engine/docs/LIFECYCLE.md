...
| Generated-path recovery | Only after an explicit acquire, fence, or backend failure | Zero-time checks in normal recovery; after acquire backoff, one image uses the configured acquire budget or one confirmed display period, and is nonblocking if neither exists, **low to bounded high** | Failure returns to native drain and increases backoff. Success resumes after fresh history without inventing a performance classification |
...
| Lower presentation | Every delivered image | Driver/compositor queue-present call, **potentially unbounded outside MAKO's control** | Returned duration is diagnostic only. MAKO cannot cancel a currently blocked driver call and does not infer recovery authority after it returns |
...
| Smooth Cadence retry | Normally 15 seconds; 60 seconds per ordered FIFO handoff rung | Prevent repeated pacing-mode churn without blocking a separately validated rung |
...
| Smooth Cadence rescue | 1 second real-only measurement; 15-second rescue cooldown | Requalify a user-enabled constant-cadence policy without changing ordinary Fractional or Fixed gameplay |
...
| Generated-image drain | 250, 500, 1000, 2000, 5000, 15000, then 30000 ms after an explicit acquire failure | Pace bounded retries after a proven Vulkan failure; successful call duration and native FPS have no authority |
...
4. Requests the application minimum image count plus generated capacity when allowed. Ordered Frame-Generation-only paths may reserve one relief image; combined scaling that is active or configured above 1.0 omits that relief reservation for compatibility. An enabled 1.0 scaler remains inactive and retains the FG-only relief topology. The surface maximum remains authoritative, and Game Swapchain Images compatibility preserves the application's original minimum from process start.
...
Smooth Cadence is an opt-in Adaptive policy that looks for a constant integer multiplier which can satisfy demand without excessive overshoot. It requires stable evidence before entry, evaluates load for one second, and exits after sustained loss of eligibility. Its ordered 2x handoff is allowed when the target matches confirmed refresh within the tighter of one hertz or two percent, including under VRR. Requested or active VRR may also hand a validated 3x–5x rung to FIFO as a full-batch pacing experiment when its projected output is within 98–125% of target. Fractional Adaptive, unvalidated higher work, and Steady operation outside these handoffs retain MAKO's target clock under VRR.
...
MAKO separates explicit transitions, direct transport failure, and feature policy. Confirmed Steam UI focus suspends generated work and scheduler observations while preserving real/scaled presentation. Return refreshes three history frames, retains the proven generated level, and arms one recreation permission that is consumed by the first successful generated-image acquire. Unknown focus, an unproven policy after a live reset, and unsupported transports retain conservative settling but cannot authorize recreation. A second interruption restarts the full history warm-up in both Fixed and Adaptive modes. Focus changes require 250 ms agreement; samples expire after one second without monitor updates.
...
2. **Direct acquire failure:** only `VK_TIMEOUT`, `VK_NOT_READY`, or exhaustion of the configured cumulative application-present budget starts native backoff. Successful-call duration, FPS, cadence, resolution, scaling, and shader cost are ignored.
3. **Bounded retry:** after 250, 500, 1000, 2000, 5000, 15000, then 30000 ms, fresh history precedes one bounded generated-image acquire. A real-only policy frame leaves the retry pending without acquisition or a failure count. Success resumes the selected policy; failure returns to backoff.
4. **Transition-scoped recreation:** a confirmed menu return or explicit live generation-policy change arms one permission. A successful acquire consumes it. A real acquire failure may consume it by requesting one retirement-protected game-owned recreation; there are no timed windows, FPS qualifiers, or failure-count thresholds.
5. **Backend recovery:** a render-fence miss or backend failure switches to real frames while readiness is polled with zero-time waits. This is direct synchronization safety and cannot select a multiplier or request recreation. Recovery resets temporal history before generation resumes.

Normal ordered generated-image acquisition is unbounded when no compatibility timeout is configured. This avoids false timeouts on healthy WSI paths with reserved headroom, but it is the largest MAKO-controlled blocking exposure: a stuck driver/compositor acquire can hold that present call until the driver returns. A configured acquire budget bounds the cumulative waits and is the only normal-path deadline; headroom-tight paths use nonblocking admission regardless. The lower queue-present call itself is always driver-controlled and cannot be bounded by MAKO, and its returned duration is diagnostic only.
...
| Gamescope VRR pacing ownership | Keep explicit requested or active VRR out of the higher-multiplier fixed-refresh cap ladder while allowing validated integer FIFO handoffs | Verified server-zero properties; Fixed and proven Steady 2x can use FIFO without assuming a periodic scanout clock; live changes reset only affected pacing helpers | **Explicit compositor contract, not a performance heuristic.** It cannot select a multiplier, start recovery, or request recreation |
| Unbounded normal ordered acquisition | Avoid rejecting healthy but variably delayed compositor acquisition | Reserved headroom and an optional explicit cumulative timeout; elapsed successful-call duration has no policy authority | **Review-worthy.** It preserves compatibility but cannot protect the current call from a driver stall |
