...
For MAKO Decky, select the intended game profile, open **Spatial Scaling**, and enable the feature before launching the game so Decky can stage the verified Gamescope WSI-before-MAKO presentation path. Then choose an in-game resolution below the display resolution and select the method, factor, and sharpness. Inside that provisioned process, the enable switch and every scaler control use one Vulkan-standard swapchain-recreation request after edits settle, so a brief flicker is normal. A process launched with Scaling off remains on the isolated fixed path; its first live off-to-on change is saved but requires one game restart because Vulkan cannot insert Gamescope WSI into an existing instance. For direct Renderer configuration, set the `scaling_*` fields in the [Configuration](#configuration) example below. `frame_generation_enabled = false` activates scaling-only; enabling it provisions LSFG so the same reconstructed real frame can feed Fixed or Adaptive generation. Changing from a scaling-only process to Frame Generation still requires a complete process restart because swapchain recreation cannot add device features.
...
The extent-policy revision changes only when scaling enablement, factor, or process support changes. Method and sharpness edits can therefore reuse the same fixed extent contract during a game-owned live recreation. An enablement/factor/support change requires a fresh capability query before a fixed source request can activate; stale low-resolution requests fail closed, while a native request remains native.
