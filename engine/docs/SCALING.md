...
A skipped GPU test is not evidence of scaling correctness. MAKO Gym's procedural visual matrix validates deterministic pixels, its timestamp-query suite measures the offscreen production scaler graph, its synchronization suite detects selected offscreen access hazards, its native Gamescope E2E suite validates WSI presentation and live lifecycle behavior, and its Proton E2E suite validates deterministic D3D11/DXVK plus D3D12/VKD3D-Proton scenes through the production chain; none replaces subjective captures, power/latency measurement, or the real title/runtime matrix.

Scaling teardown preserves the ordinary replacement contract: a retired lower swapchain remains alive through a later replacement present, its maintenance1 fence, and the compositor grace. Because terminal D3D12/VKD3D-Proton shutdown may destroy the final swapchain without issuing another present, MAKO associates every deferred lower swapchain with its creating surface. Explicit surface destruction proves no replacement on that surface can follow, so MAKO synchronously drains the same maintenance1 fence and releases the lower swapchain before forwarding `vkDestroySurfaceKHR`. This path adds no background thread or steady-state polling and cannot bypass an unsignaled fence; MAKO Gym's Proton lane requires deferred/completed terminal retirement plus client and Gamescope-child termination without forced cleanup.
