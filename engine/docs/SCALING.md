...
Run `scripts/run-mako-gym.sh` with the private sibling MAKO-Gym checkout for the advanced native-Vulkan smoke lane. Gym owns its 44-case declarative inventory, headless Gamescope orchestration, runtime assertions, logs, and `summary.tsv`; MAKO owns only the optional/required bridge and its portable contract test. The cases cover every scaler and LS1 learned sharpness variant, explicit fallback, scaling-only resource separation, isolated Fixed/Adaptive/Steady/Smooth/recovery/Ultra policies, FP32/FP16 configurations, Flow Scale and lighter-model states, and representative combined paths. The complete lane breakdown, duration classes, pass contract, controls, and evidence limits are authoritative in `MAKO-Gym/docs/VULKAN-FEATURE-MATRIX.md`; [Testing MAKO](../../TESTING.md#mako-gym-advanced-native-vulkan-smoke-matrix) owns when that private gate is optional or required. Neither lane replaces the image-quality, API, game, live-mutation lifecycle, driver, and hardware rows below.
...
A skipped GPU test is not evidence of scaling correctness. MAKO-Gym's LSFG image-quality regression validates the proprietary frame-generation path and does not replace spatial reconstruction captures, combined-pipeline performance evidence, or the real game/runtime matrix.
