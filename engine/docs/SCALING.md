...
For MAKO Decky, select the intended game profile, open **Spatial Scaling**, enable **Scaling**, and launch or restart the game so Decky can stage the verified split path: the MAKO frame-generation role, Gamescope WSI, then the MAKO spatial-scaling role. Inside that provisioned process, Native Resolution, MAKO Scaler, LS1 Quality, and LS1 Performance share one immutable source/presentation extent contract. You can change the model while the game is running; method changes rebuild only the lower role's private scaler at the next presentation boundary, while the game swapchain, Gamescope WSI object, and layer chain remain untouched. Sharpness changes use the same private transition after a 500 ms quiet period so slider edits coalesce. Scale Factor changes the extent contract and therefore waits for the game's next natural resolution change or restart. Turning Scaling on or off is always saved for the next process and never changes the running process's layer membership. For direct Renderer configuration, set the `scaling_*` fields in the [Configuration](#configuration) example below and restart after changing `scaling_enabled`; direct launch tooling retains the single-layer combined behavior and does not automatically stage Decky's managed split path. `frame_generation_enabled = false` runs scaling alone without generation work, while the matched upper role retains the interop and private resources required to enable Fixed or Adaptive Frame Generation live.
...
| Lifecycle | Initial create, focus and overlay transitions, private Native/MAKO/LS1 model and sharpness changes with zero application-visible recreation, Scale Factor deferral, game-owned resolution change, natural swapchain recreation, process-restart scaling changes, profile update, and shutdown. |
