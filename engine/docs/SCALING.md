...
For MAKO Decky, select the intended game profile, open **Spatial Scaling**, enable **Scaling Engine (Restart)**, and launch or restart the game so Decky can stage the verified split path: the MAKO frame-generation role, Gamescope WSI, then the MAKO spatial-scaling role. Inside that provisioned process, Native Resolution, MAKO Scaler, LS1 Quality, and LS1 Performance share one immutable source/presentation extent contract. Method changes rebuild only the lower role's private scaler at the next presentation boundary; the game swapchain, Gamescope WSI object, and layer chain remain untouched. Sharpness changes use the same private transition after a 500 ms quiet period so slider edits coalesce. Scale Factor changes the extent contract and therefore waits for the game's next natural resolution change or restart. Turning Scaling Engine on or off is always saved for the next process and never changes the running process's layer membership. For direct Renderer configuration, set the `scaling_*` fields in the [Configuration](#configuration) example below and restart after changing `scaling_enabled`; direct launch tooling retains the single-layer combined behavior and does not automatically stage Decky's managed split path. `frame_generation_enabled = false` runs scaling alone without generation work, while the matched upper role retains the interop and private resources required to enable Fixed or Adaptive Frame Generation live.
...
MAKO's default method remains fully repository-owned. Its embedded compute shader performs a clamped Catmull-Rom reconstruction, contrast-aware anti-ringing, and bounded local sharpening with a static twofold baseline that `scaling_sharpness` multiplies. The contrast-relative envelope scales with that baseline while remaining bounded to prevent uncontrolled halos. It is also the automatic runtime fallback when LS1 cannot find the licensed DLL or translator, the DLL lacks the expected resources, or the driver rejects the LS1 formats or pipelines. The active and requested methods plus any fallback reason are logged explicitly.
...
The spatial stage allocates no additional GPU resources when Scaling Engine is disabled or the selected swapchain is unsupported. Native Resolution owns one source-sized and one presentation-sized transfer image and records linear transfer blits, but allocates no shader, descriptor, parameter buffer, licensed resource, or compute dispatch. On the measured RADV NAVI33 development GPU, its complete 640×360→1280×720 graph took 44.12–48.48 μs across five timestamped samples and passed the procedural traffic pixel thresholds. Independently, every matched process retains the LSFG interop/backend resources described above so Frame Generation can switch live; those resources perform no per-frame work while Off. MAKO and LS1 additionally own their method-specific sampled/storage images, immutable parameters, pipelines/descriptors, and one reusable command buffer and ready semaphore per WSI image.
...
| `scaling_sharpness` | 0.0–1.0 | `0.5` | Multiplier for MAKO's continuous sharpening at its static 2x baseline, or LS1's nearest one of five learned model variants. |
...
Scaling Engine enablement cannot mutate process-start Vulkan layer membership and always waits for game restart. With the engine provisioned, Native Resolution, MAKO Scaler, LS1 Quality, and LS1 Performance share immutable application and presentation extents. Context creation resolves and retains the configured or discovered licensed DLL path even when Native Resolution or MAKO Scaler is initially selected, so a later private LS1 selection does not depend on rebuilding application-visible WSI state. A method selection constructs the replacement pipeline before touching the active one, waits for application-device work to become idle only on that user-driven transition, swaps the private resources, warms frame-generation history, and leaves the game swapchain plus Gamescope WSI object untouched. Sharpness uses the same transition after a 500 ms quiet period. A failed replacement retains the previous scaler and retries after five seconds; LS1 discovery or translation failure still produces the documented MAKO fallback. Scale Factor changes the application-visible extent contract and therefore waits for the game's next natural swapchain recreation or process restart. Flow Scale, Lighter FG Model, pacing shape, and generated-frame capacity remain at their actually constructed values until their documented natural-recreation or restart boundary. MAKO does not synthesize an out-of-date result for spatial model switching in the managed Gamescope lane. Natural resolution changes still use maintenance1 fences, exact same-surface lower-handle replacement, the compositor grace, and surface-terminal retirement to protect lower WSI lifetimes. For a combined write containing a process-static change such as Scaling Engine, Ultra Performance, or GPU selection, Root retains the running process's static baseline while applying compatible live fields and reporting the remaining boundary. Frame Generation itself turns on and off immediately because every successfully provisioned matched process retains the required interop/backend resources. Other live Fixed/Adaptive settings retain the semantics documented in [Configuration](CONFIGURATION.md).
...
- process-static Scaling Engine projection, private Native/MAKO/LS1 method rebuilds, debounced private sharpness rebuilding, and natural-recreation deferral for extent/backend/capacity changes; and
- GLSL, SPIR-V payload, and hash freshness for both SDR working formats.

Run `scripts/run-mako-gym.sh` with the private sibling MAKO Gym checkout for the hardware lanes. Gym's native-Vulkan inventory owns headless Gamescope orchestration, runtime assertions, logs, and `summary.tsv`; it covers Scaling Engine provisioned with the Native Resolution linear baseline, every scaler and LS1 learned sharpness variant, explicit fallback, live Frame Generation Off→On resource retention, isolated Fixed/Adaptive/Steady/Smooth/recovery/Ultra policies, FP32/FP16 configurations, Flow Scale and lighter-model states, and representative combined presentation paths. Gym's separate procedural visual inventory runs its scenes through real LSFG, every production spatial method, edge factors/timestamps, and the actual scaling-to-LSFG handoff while retaining validated PPM comparisons. Repeated sentinels detect nondeterministic output, repeated LSFG workloads enforce practical throughput and variance budgets through high resolutions, every-scaler row scores exact-resolution pixels and timestamps the complete scaler graph, paired live rows measure present/full-frame p95/p99 plus CPU/RSS overhead, and canonical paths run under Khronos synchronization validation. The release-only `gamescope-e2e` lane loads the exact frame-generation-role → WSI → spatial-scaling-role chain and must prove private Native/MAKO/LS1 transitions without application-visible recreation, FG Off/On, noisy high-base Adaptive recovery, natural resolution replacement, stable 1080p→4K followed by 1440p→4K extent retention, one compound transition journey, scaling-disabled top-only isolation, and missing-dependency fail-closed behavior. The separate release-only `proton-e2e` lane runs all deterministic game-like scenes through D3D11/DXVK and D3D12/VKD3D-Proton while requiring the requested spatial path and positive Fixed/Adaptive generated delivery; `proton-compatibility` repeats stratified canonical scaling/generation sentinels across recorded official, Experimental, Hotfix, and GE Proton families. The complete current counts and contracts are authoritative in `MAKO-Gym/docs/`; [Testing MAKO](../../TESTING.md#mako-gym-advanced-native-vulkan-smoke-matrix) owns when each private gate is optional or required. These lanes do not replace title-specific game compatibility, subjective review, another driver, or another hardware class.
...
| Lifecycle | Initial create, focus and overlay transitions, private Native/MAKO/LS1 model and sharpness changes with zero application-visible recreation, Scale Factor deferral, game-owned resolution change, natural swapchain recreation, process-restart Scaling Engine changes, profile update, and shutdown. |
