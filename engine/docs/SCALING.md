...
This is the Steam Deck/internal-display and 720p-to-1080p path. With `frame_generation_enabled = false`, the reconstructed real frame goes directly to the lower presentation path. With Fixed or Adaptive Frame Generation enabled, the same reconstructed result becomes the exported LSFG source and the backend generates at the presentation resolution. Generated frames are not scaled individually, so increasing the multiplier does not multiply spatial work.
...
Desktop window systems normally publish `UINT32_MAX` as `currentExtent`, so there is no native size to advertise during the capability query. MAKO preserves the application's requested extent as the source and requests a lower presentation extent enlarged by `scaling_factor`. It uses one aspect-preserving effective factor for both dimensions, clamps that factor to the Vulkan surface maximum extent, then rounds enlarged dimensions down to even values. In a Gamescope process, the existing server-zero feedback resolver also reads the proven root geometry and applies it as the first presentation ceiling. A 960×540 source requested at 1.8× on a 1280×800 output therefore selects 1280×720, while a 1280×720 source has no aspect-preserving enlargement headroom and stays native with `inactive_reason=gamescope-presentation-target-no-headroom`. Explicit `scaling_supersampling = true` bypasses only this output-target ceiling, so the 960×540 example may select 1726×970; Vulkan surface limits and the deterministic memory envelope below remain mandatory. The lower split role requires the positive Gamescope target even when supersampling is enabled, preserving the ordered Frame Generation → Gamescope WSI → Spatial Scaling proof instead of turning the option into an absent-contract bypass. An unavailable target fails closed with `inactive_reason=gamescope-presentation-target-unavailable`, and the lower create relay remains authoritative if the upper role sampled earlier metadata. Direct non-Gamescope variable surfaces retain the factor-based behavior without inventing a monitor target, so supersampling does not change their geometry. Before allocating a remaining variable lower swapchain, MAKO checks the exact candidate against a device-memory presentation envelope derived from the largest device-local heap. The envelope retains a 3840×2160 floor for unified-memory devices and otherwise budgets one presentation pixel per 768 heap bytes: one third of the heap against a conservative 256-byte-per-pixel combined active/retired spatial-plus-frame-generation estimate. If the configured factor exceeds that envelope, MAKO deterministically aspect-fits the source into the 3840×2160 baseline instead of borrowing a prior swapchain's extent. A cold 2560×1440 launch and a 1920×1080→2560×1440 transition therefore both select 3840×2160 at an effective 1.5× on an 8 GiB device, rather than attempting 5120×2880 or changing scaling activity according to launch history. A later 2.0×→1.8× edit remains above that active 1.5× envelope, so it updates the requested/applied profile without recreating the swapchain or scaler; an edit below 1.5× still takes the ordinary extent boundary. If the source cannot enlarge both dimensions inside that baseline, the candidate stays native with `inactive_reason=variable-surface-memory-budget`. This blocks 4K-to-8K on an 8 GiB device while allowing 1080p-to-4K there, preserves non-widescreen ratios, and admits exact 4K-to-8K on a 24 GiB device. Fixed surfaces already own their actual presentation extent and do not use the variable-surface target guard, supersampling override, or its no-op optimization.
...
After either method, MAKO copies the reconstructed image across the complete WSI image and returns it to `VK_IMAGE_LAYOUT_PRESENT_SRC_KHR` before the lower present. On the pre-FG path it also copies the same presentation-sized result into the exported LSFG source. On the post-FG path it instead reconstructs each source-sized private LSFG destination directly into the acquired presentation-sized WSI image. Explicit compute-to-compute barriers separate LS1's learned stages; all images, pipelines, descriptors, and constants are created once with the swapchain.
...
| Lifecycle | Initial create, focus and overlay transitions, private Native/MAKO/LS1 model and sharpness changes with zero application-visible recreation, simultaneous or superseded private FG model/Flow/capacity changes, coalesced Scale Factor and Quality Supersampling requests with exactly one retirement-fenced out-of-date signal when effective extents change, fixed/direct and unchanged-extent no-ops, combined factor plus Fixed multiplier/capacity and factor plus Fixed-to-Adaptive ceiling/capacity changes, unsupported-path extent deferral, game-owned resolution change, natural and requested swapchain recreation, process-restart scaling changes, profile update, and shutdown. |
