...
MAKO Renderer provides an optional Vulkan spatial-scaling stage that is independent from frame synthesis. A game renders a smaller source image, MAKO reconstructs it to the presentation extent on the application's Vulkan device, and the full-resolution real frame is then either presented directly or supplied to Fixed or Adaptive Frame Generation. Scaling is disabled by default and offers MAKO's open single-pass method, the proprietary LS1 Quality neural network, and the lower-cost LS1 Performance neural network.
...
For MAKO Decky, select the intended game profile, choose an in-game resolution below the display resolution, open **Spatial Scaling**, enable the feature, and select the method, factor, and sharpness. A running game receives one Vulkan-standard swapchain-recreation request after edits settle, so a brief flicker is normal and no process restart is normally needed. For direct Renderer configuration, set the `scaling_*` fields in the [Configuration](#configuration) example below. `frame_generation_enabled = false` activates scaling-only; enabling it provisions LSFG so the same reconstructed real frame can feed Fixed or Adaptive generation. Changing from a scaling-only process to Frame Generation still requires a complete process restart because swapchain recreation cannot add device features.
...
Application source rectangle
        |
        v
selected spatial reconstruction (MAKO or LS1)
        |
...
Gamescope and other fixed-extent surfaces publish a concrete `currentExtent`. After transfer-usage and format-feature preflight, MAKO intercepts both surface-capability entry points and advertises `floor(presentation / scaling_factor)` as the source extent. Dimensions greater than one are rounded down to even values so compute and frame-generation workgroups avoid half-pixel asymmetry. The lower swapchain retains the compositor-owned presentation extent.

MAKO scales only when the application's swapchain request matches the source extent it advertised. An application-selected override is not silently replaced with a different source size. The original lower capabilities are queried again at swapchain creation and remain authoritative for presentation.
...
The spatial stage allocates no additional GPU resources when it is disabled or when the selected swapchain is unsupported. A process that starts in scaling-only mode also omits all frame-generation interop and private-backend resources. Every active method owns one source-sized sampled image, one presentation-sized output image, immutable parameters, pipelines/descriptors, and one reusable command buffer and ready semaphore per WSI image.
...
| --- | --- | --- | --- |
| `scaling_enabled` | Boolean | `false` | Requests the spatial stage for supported swapchains. |
| `scaling_method` | `mako`, `ls1`, `ls1-performance` | `mako` | Selects the open single-pass method, LS1 Quality, or LS1 Performance. LS1 requires the licensed DLL and vkd3d-shader; failure uses MAKO and logs why. |
...
- fixed-surface capability virtualization, source rounding, exact create matching, variable-surface enlargement, echoed-presentation feedback suppression, maximum-extent clamping, disabled and 1.0-factor behavior;
- opaque single-array-layer, unprotected, non-shared-present swapchain-shape policy;
- debounced one-shot live-recreation state for scaling enablement, method, factor, sharpness, Flow Scale, and Lighter FG Model changes, including cancellation, edit coalescing, and no repeated signal for the same request; and
- GLSL, SPIR-V payload, and hash freshness for both SDR working formats.

Run `scripts/run-mako-gym.sh` with the private sibling MAKO-Gym checkout for both hardware lanes. Gym's 44-case native-Vulkan inventory owns headless Gamescope orchestration, runtime assertions, logs, and `summary.tsv`; it covers every scaler and LS1 learned sharpness variant, explicit fallback, scaling-only resource separation, isolated Fixed/Adaptive/Steady/Smooth/recovery/Ultra policies, FP32/FP16 configurations, Flow Scale and lighter-model states, and representative combined presentation paths. Gym's separate 66-case procedural visual inventory runs five scenes through real LSFG, every production spatial method, and the actual scaling-to-LSFG handoff while retaining validated PPM comparisons. The complete live lane is authoritative in `MAKO-Gym/docs/VULKAN-FEATURE-MATRIX.md`; the visual lane is authoritative in `MAKO-Gym/docs/AMD-QUALITY-REGRESSION.md`; [Testing MAKO](../../TESTING.md#mako-gym-advanced-native-vulkan-smoke-matrix) owns when each private gate is optional or required. Neither lane replaces API/game compatibility, live-mutation lifecycle, performance, subjective review, another driver, or another hardware class.
...
| Lifecycle | Initial create, focus and overlay transitions, resolution change, natural swapchain recreation, live scaling enable/disable/method/factor/sharpness recreation, ignored one-shot compatibility fallback, profile update, and shutdown. |
...
Use the Vulkan validation layer when it is installed and preserve `VK_LOADER_DEBUG=layer` evidence that the development build, rather than an older installed layer, was activated. An implicit MAKO layer must be selected through its development manifest path; setting only the explicit-layer path is not proof. Inspect `MAKO Renderer: spatial scaling surface virtualized`, `spatial scaling swapchain policy`, and `spatial scaling active` records for the selected dimensions, factor, format, and active decision. Live resource changes emit `runtime-transition-pending` with separate scaling, flow-resolution, and lighter-model fields, followed by `runtime-transition-recreation-requested`; the replacement context's `runtime-state-applied` record exposes `effective_flow_scale` and `lighter_model`.

A skipped GPU test is not evidence of scaling correctness. MAKO-Gym's procedural visual matrix validates deterministic pixels for spatial reconstruction and combined frame generation, but it does not replace GPU profiling, subjective captures, WSI presentation, or the real game/runtime matrix.
