...
| --- | --- | --- | --- |
| `native` — **Native Resolution** | Passthrough; no spatial reconstruction graph | No spatial-model DLL or shader translator | Default live A/B baseline that keeps the process-start Scaling Engine/WSI lane provisioned without allocating or dispatching a spatial scaler. |
| `mako` — **MAKO Scaler** | Repository-owned one-pass Catmull-Rom reconstruction, contrast-aware anti-ringing, and bounded sharpening | No spatial-model DLL or shader translator | Smallest resource and pass count; also the automatic per-swapchain fallback when LS1 setup fails. |
...
Every matched process negotiates the external-memory, external-semaphore, and timeline-semaphore features used by LSFG and constructs its private frame-generation backend and generated-output resources when startup succeeds, even with `frame_generation_enabled = false`. Native Resolution creates no spatial pipeline; MAKO Scaler uses only repository-owned spatial shaders; LS1 locates the user's DLL and translates the selected spatial graph. While Frame Generation is Off, the per-frame path bypasses the retained LSFG resources completely. This deliberate startup-memory tradeoff lets the switch enable live without attempting to mutate device features or rebuild the game swapchain. If startup interop or backend construction failed, the Renderer keeps native or independent scaling available and reports that generation requires restart.
...
The spatial stage allocates no additional GPU resources when Scaling Engine is disabled, Native Resolution is selected, or the selected swapchain is unsupported. A provisioned Native Resolution process still pays the compatibility and presentation behavior of the Gamescope WSI layer chain described in [Gamescope WSI isolation](WSI-ISOLATION.md), but MAKO performs no spatial allocation, copy, shader dispatch, or licensed spatial-resource lookup. Independently, every matched process retains the LSFG interop/backend resources described above so Frame Generation can switch live; those resources perform no per-frame work while Off. Every active scaler owns one source-sized sampled image, one presentation-sized output image, immutable parameters, pipelines/descriptors, and one reusable command buffer and ready semaphore per WSI image.
...
| `scaling_method` | `native`, `mako`, `ls1`, `ls1-performance` | `native` | Selects Native Resolution passthrough, MAKO Scaler, LS1 Quality, or LS1 Performance. Method changes are live through game-owned recreation after the engine is provisioned. LS1 requires the licensed DLL and vkd3d-shader; failure uses MAKO Scaler and logs why. |
...
Scaling Engine enablement cannot mutate process-start Vulkan layer membership and always waits for game restart. With the engine provisioned, changing Native Resolution/scaler method, factor, or sharpness, changing Flow Scale, selecting Lighter FG Model, or selecting a Fixed/Adaptive policy that needs more generated-frame capacity cannot mutate the GPU resources owned by an existing game swapchain. A discrete method selection requests recreation on the next retirement-protected lower present so a short return to gameplay cannot be overtaken by another Quick Access edit. Numeric factor/sharpness controls, Flow Scale, Lighter FG Model, and capacity changes retain a 500 ms quiet period to coalesce slider or grouped edits. At the selected boundary, MAKO converts only a successful or suboptimal protected lower result into one `VK_ERROR_OUT_OF_DATE_KHR`. MAKO requests KHR or EXT swapchain-maintenance1 at application-device creation only when the physical device advertises the matching extension and feature. Gamescope's non-null upstream present fence is preserved as the completion proof when supplied; otherwise MAKO attaches its own preallocated maintenance1 fence. A conforming Vulkan game requests destruction and recreation of its swapchain, and the normal creation path applies the latest scaling and LSFG context profile. MAKO removes the retired context from live updates immediately, but retains the lower WSI resources until a later replacement present, a 50 ms compositor grace, and all associated MAKO-owned per-image maintenance fences complete. Multiple edits settle into the same request. MAKO never initiates the application's destroy request, never takes ownership of an upstream fence, never signals without a valid lower-present retirement proof, and never repeats the same request; a brief flicker is expected. If maintenance1 is unavailable or its fence is invalid, live resource recreation falls back to a natural game-owned boundary. Method and sharpness changes retain the compatible fixed extent contract. Factor changes require the title to make a fresh surface-capability query; if it recreates directly at its native fixed extent, the scaler stays inactive for that process. A title that ignores the one-shot result retains its current context and needs a process restart as the compatibility fallback for changes that depend on recreation. Flow Scale, Lighter FG Model, and generated-frame capacity use this live path when frame-generation startup resources are available. For a combined write containing a process-static change such as Scaling Engine, Ultra Performance, or GPU selection, Root retains the running process's static baseline while applying compatible fields and safe recreation requests; diagnostics keep the static portion explicitly pending for restart. Frame Generation itself turns on and off immediately because every successfully provisioned matched process retains the required interop/backend resources. Other live Fixed/Adaptive settings retain the semantics documented in [Configuration](CONFIGURATION.md).
...
- process-static Scaling Engine projection plus immediate discrete Native/scaler method recreation and debounced numeric/model/capacity one-shot recreation, including cancellation, edit coalescing, and no repeated signal for the same request; and
- GLSL, SPIR-V payload, and hash freshness for both SDR working formats.

Run `scripts/run-mako-gym.sh` with the private sibling MAKO Gym checkout for the hardware lanes. Gym's native-Vulkan inventory owns headless Gamescope orchestration, runtime assertions, logs, and `summary.tsv`; it covers Scaling Engine provisioned with Native Resolution passthrough, every scaler and LS1 learned sharpness variant, explicit fallback, live Frame Generation Off→On resource retention, isolated Fixed/Adaptive/Steady/Smooth/recovery/Ultra policies, FP32/FP16 configurations, Flow Scale and lighter-model states, and representative combined presentation paths. Gym's separate procedural visual inventory runs its scenes through real LSFG, every production spatial method, edge factors/timestamps, and the actual scaling-to-LSFG handoff while retaining validated PPM comparisons. Repeated sentinels detect nondeterministic output, repeated LSFG workloads enforce practical throughput and variance budgets through high resolutions, every-scaler rows score exact-resolution pixels and timestamp complete scaler graphs, paired live rows measure present/full-frame p95/p99 plus CPU/RSS overhead, and canonical paths run under Khronos synchronization validation. The release-only `gamescope-e2e` lane loads the real WSI layer above MAKO and proves Native/MAKO/LS1 live transitions, FG Off/On, maintenance1-fenced retirement, natural recreation, and reversed-order fail-closed behavior through the real compositor. The separate release-only `proton-e2e` lane runs all six deterministic game-like scenes through both D3D11/DXVK and D3D12/VKD3D-Proton while requiring the requested spatial path and positive Fixed/Adaptive generated delivery; `proton-compatibility` repeats stratified canonical scaling/generation sentinels across four recorded Proton families. The complete current counts and contracts are authoritative in `MAKO-Gym/docs/`; [Testing MAKO](../../TESTING.md#mako-gym-advanced-native-vulkan-smoke-matrix) owns when each private gate is optional or required. These lanes do not replace title-specific game compatibility, subjective review, another driver, or another hardware class.
...
| --- | --- |
| Surface ownership | Fixed Gamescope surface with HDR exposure disabled and variable desktop surface; source and presentation extents must match the policy logs, an echoed variable presentation extent must fall back without recursive enlargement, and a variable candidate beyond the device-memory envelope must remain native. Cover Vulkan-maximum clamping, 4:3 or 5:4 preservation, ultrawide, 4K, 5K rejection on an 8 GiB tier, and an admitted 8K row on a 24 GiB tier. |
...
Use the Vulkan validation layer when it is installed and preserve `VK_LOADER_DEBUG=layer` evidence that the development build, rather than an older installed layer, was activated. An implicit MAKO layer must be selected through its development manifest path; setting only the explicit-layer path is not proof. Inspect `MAKO Renderer: spatial scaling surface virtualized`, `spatial scaling swapchain policy`, and `spatial scaling active` records for the selected dimensions, factor, format, and active decision. The policy record exposes fixed/variable surface mode, `device_local_heap_mib`, `variable_presentation_pixel_budget`, advertised and actual source/presentation extents, current and contract policy revisions, query generation, the stable `inactive_reason`, and `source_presentation_split`. `inactive_reason=variable-surface-memory-budget`, `active=0`, or `source_presentation_split=0` proves that no scaler ran even if a model was selected in configuration. Live resource changes emit `runtime-transition-pending` with separate scaling, flow-resolution, and lighter-model fields, followed by `runtime-transition-recreation-requested`; the replacement context's `runtime-state-applied` record exposes `effective_flow_scale` and `lighter_model`.

A skipped GPU test is not evidence of scaling correctness. MAKO Gym's procedural visual matrix validates deterministic pixels, its timestamp-query suite measures the offscreen production scaler graph, its synchronization suite detects selected offscreen access hazards, its native Gamescope E2E suite validates WSI presentation and live lifecycle behavior, its Proton E2E suite validates deterministic D3D11/DXVK plus D3D12/VKD3D-Proton scenes through the production chain, and its Proton compatibility suite repeats canonical scaling/generation sentinels across recorded runtime families; none replaces subjective captures, power/latency measurement, or the real title/runtime matrix.

Scaling teardown preserves the ordinary replacement contract: a retired lower swapchain remains alive through a later replacement present, its maintenance1 fence, and the compositor grace. If an upper WSI destroys before recreating and supplies a null lower `oldSwapchain`, MAKO hands the exact retained same-device, same-surface lower handle to `vkCreateSwapchainKHR` once; this lets Vulkan retire the occupied lower window association without changing application-visible ownership or releasing the retained lifetime proof. Because terminal D3D12/VKD3D-Proton shutdown may destroy the final swapchain without issuing another present, MAKO associates every deferred lower swapchain with its creating surface. Explicit surface destruction proves no replacement on that surface can follow, so MAKO synchronously drains the same maintenance1 fence and releases the lower swapchain before forwarding `vkDestroySurfaceKHR`. This path adds no background thread or steady-state polling and cannot bypass an unsignaled fence; MAKO Gym's Proton lane requires deferred/completed terminal retirement plus client and Gamescope-child termination without forced cleanup.
