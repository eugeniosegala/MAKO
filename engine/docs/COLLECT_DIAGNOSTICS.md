...
For Scaling with Gamescope WSI disabled, the `scaling` preset includes `spatial scaling surface bridge` records. `transport=wayland; gamescope_wsi=isolated` proves the minimal Gamescope surface association; an accompanying active source/presentation split is still required to prove that scaling ran.
