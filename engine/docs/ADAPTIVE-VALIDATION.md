...
The Adaptive surface watchdog requires a confirmed Steam focus round trip and a one-second baseline at or above 95% of target, last observed within 15 seconds before menu opening. Long menu visits preserve that baseline. After return, four continuous seconds below 75% of target with at least 50% lower-present time can qualify a request; warm-up, probes, transport recovery, failed presents, and missing measurements cannot qualify. A return episode expires after 75 seconds and allows at most two watchdog requests, at least 30 seconds apart. The baseline survives MAKO-requested same-policy replacement for one bounded retry. Changed settings, refresh, extents, unknown/stale focus, Off/ineligible modes, and unrequested replacement invalidate it. One second above the deficit band ends the episode. Requests require a successful retirement-fenced lower present, and the game owns replacement. A heavier scene after a real menu return can still resemble failed recovery: lower-present time is not proof of a wedged compositor.
