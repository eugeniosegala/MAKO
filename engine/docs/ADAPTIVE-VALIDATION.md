...
Ordered Acquire Recovery is separately owned by the presentation transport and applies to both Adaptive and Fixed generation. One isolated slow generated-image acquire remains ordinary FIFO backpressure. A configured acquire timeout, or two consecutive acquisitions exceeding the greater of 25 ms and one-and-a-half display periods, quarantines generated work instead of letting the next application present repeat the same blocked acquire. MAKO presents natively for 250 ms, warms three temporal-history frames, and makes exactly one generated recovery probe, even when the retained policy is 3x or 4x; that probe, and a following two-second stabilization window, acquire only with a zero timeout and use one generated frame at most. An unavailable image therefore remains native-only instead of consuming another 50 ms application-thread wait. Adaptive freezes cadence, ramp, load-shed, and delivery evaluation until the stabilization window is healthy. Repeated initial pressure backs off to 500 ms, one second, and then a bounded two seconds. A healthy window resumes the retained policy and resets the retry history. The drain performs no backend scheduling or synthetic swapchain acquisition, so a Steam overlay transition can release ordered-FIFO images without destroying the game-owned swapchain or resetting the validated Adaptive multiplier.
