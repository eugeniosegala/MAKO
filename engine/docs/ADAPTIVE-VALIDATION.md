...
Use `scripts/run-mako-gym.sh` with the private sibling MAKO-Gym checkout as the repeatable native-Vulkan bridge between deterministic scheduler tests and the game matrix. Gym's 44-case finite-cube suite owns broad Fixed, Fractional Adaptive, Steady Adaptive with Smooth Cadence, Dynamic Cadence Recovery configuration, Ultra Performance, and scaling-combination construction and delivery checks. It does not claim a recovery event from a steady cube. Select `--suite recovery` for Gym's separate scripted moving workload: its 12 cases control 30→60 and 30→12 FPS transitions, a false native-cadence probe, 150 ms and 500 ms stalls, and Vulkan `oldSwapchain` replacement, then assert the exact bridge, refresh, native-plan, probe-rejection, and post-replacement delivery records in order. These lanes prove their contracts on the current host, not subjective pacing, scanout timing, DXVK/VKD3D-Proton compatibility, generated-image starvation, device-loss recovery, or another GPU.

A cold process start retains the three-second stabilization guard because launcher and splash-screen cadence is not gameplay evidence. When the application creates a replacement with a live `oldSwapchain`, MAKO marks the new context as `replacement=1`, keeps the old context alive until the application retires it, performs the normal three-frame temporal-history warm-up, and uses the one-second recovery settling window before the measured ramp. MAKO-Gym requires Adaptive and LS1-plus-Adaptive generation to resume within its three-second post-replacement phase; a context created without `oldSwapchain` remains a cold start and cannot use this shortened path.
