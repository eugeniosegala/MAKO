...
Ordered generated-image acquisition uses one application-present deadline, not a refresh-derived slow-call threshold per generated image. Each image receives only the remaining cumulative configured budget. An unconfigured normal path retains its existing unbounded compatibility contract, while headroom-tight admission remains nonblocking.

Only an explicit `VK_TIMEOUT`/`VK_NOT_READY` result or exhaustion of the configured cumulative acquire budget starts ordered transport recovery. A successful acquire is successful regardless of elapsed time, output FPS, GPU load, device, resolution, scaling, or post-processing cost. Direct failures back off through 250 ms, 500 ms, one, two, five, 15, and 30 seconds and retry one image using the explicit configured acquire budget or one confirmed display period; without either contract the retry is nonblocking. Those values pace retries after a proven failure and never classify gameplay health. There is no FPS-based early retry, native-cadence hold, slow-acquire guard, repeated-episode counter, failure-count-derived acquire timeout, or timed recovery window.

A confirmed Steam-menu return or an explicit live generation-policy transition arms one one-shot recreation permission. The first successful generated-image acquire consumes it without action. If an actual acquire failure occurs first, one successful retirement-protected real present may request a game-owned recreation; the context cannot signal it twice, and a replacement context initializes from the current Gamescope return sequence so the same old event cannot rearm a loop. An ordinary gameplay acquire timeout still receives bounded in-place backoff but cannot demote accepted Adaptive state or authorize recreation. A timeout in an active higher-multiplier probe may reject that unvalidated probe only while the explicit transition permission is active.
