...
An isolated short acquire timeout below the slow-pressure threshold uses the same zero-wait guard without discarding validated cadence. Only a healthy unrestricted generated batch clears repeated-pressure evidence; a successful guard or temporal warm-up cannot clear it. Repeated failures back off through 250 ms, 500 ms, one, two, five, 15, and 30 seconds. Native cadence qualifies during backoff, so confirmed renewed demand after a target-satisfying menu can re-arm one probe early; a failed demand probe retains the failure count. After a confirmed Steam-menu return, three failed bounded probes spanning at least three seconds prove that the current Fixed or Adaptive ordered-acquire context exhausted in-place recovery. One successful retirement-protected lower present may then request a game-owned recreation, subject to the shared surface budget; ordinary gameplay without a fresh menu return cannot activate this escalation, and the same context cannot rearm it after a live reset. Transport failure during any unvalidated 2x–5x Adaptive probe returns to the exact previously proven multiplier, preserves escalating higher-probe backoff across the following cadence refresh, and reaches native-only recovery only when no generated level has been validated.
