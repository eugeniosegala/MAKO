...
Use `scripts/run-mako-gym.sh` with the private sibling MAKO-Gym checkout as the repeatable native-Vulkan bridge between deterministic scheduler tests and the game matrix. Gym owns the scenarios and runner; they exercise isolated Fixed, Fractional Adaptive, Steady Adaptive with Smooth Cadence, Dynamic Cadence Recovery, Ultra Performance, and representative scaling combinations in separate headless Gamescope processes, then assert active runtime state and scheduler-plan records. The finite cube workload proves construction and presentation on the current host, not subjective pacing, scanout timing, varying game cadence, DXVK/VKD3D-Proton compatibility, or another GPU.
