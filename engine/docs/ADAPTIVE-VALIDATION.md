...
Ordered Acquire Recovery is separately owned by the presentation transport and applies to both Adaptive and Fixed generation. One successful generated-image acquisition exceeding the greater of 25 ms and one-and-a-half display periods arms a zero-wait, one-generated-frame guard for the next application present. That guard prevents the transport delay from entering Adaptive's source-cadence clock and prevents the next present from repeating a blocking acquisition. An immediately available guard image resumes the retained policy on the following present without imposing a stabilization window. A guard miss, a Vulkan timeout result, a successful acquisition that exceeds its requested wall-time deadline, a successful acquisition lasting at least twice the slow threshold, or two consecutive slow acquisitions quarantines generated work. MAKO then presents natively for 250 ms, warms three temporal-history frames, and makes exactly one generated recovery probe, even when the retained policy is 3x or 4x; that drain-recovery probe, and a following two-second stabilization window, acquire only with a zero timeout and use one generated frame at most. An unavailable image therefore remains native-only instead of consuming another application-thread wait. Adaptive freezes cadence, ramp, load-shed, and delivery evaluation while recovery is constrained. Repeated initial pressure backs off to 500 ms, one second, and then a bounded two seconds. A healthy post-drain window resumes the retained policy and resets the retry history. The drain performs no backend scheduling or synthetic swapchain acquisition, so a Steam overlay transition can release ordered-FIFO images without destroying the game-owned swapchain or resetting the validated Adaptive multiplier.
