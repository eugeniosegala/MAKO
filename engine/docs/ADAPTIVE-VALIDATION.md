...
The external-recovery inventory pairs unidentified-overlay controls with confirmed Gamescope menu rows across Adaptive 2x and 3x, Fixed, Frame Generation-only, combined Frame Generation plus scaling, and scaling-only operation. Pause, covered-throttle, and repeated short/long menu journeys must independently prove source recovery, target-output recovery, focus sequencing, fresh history, generation suspension where applicable, and context stability. Portable policy tests separately permute Fractional/Smooth mode, automatic base-cap state, 3x–5x ceilings, Fixed/Adaptive watchdog ownership, recovery-history eligibility, lower-present share, and healthy/degraded returned output so hardware rows do not carry the full combinatorial burden.
