...
Adaptive ordered delivery preserves the established sequential acquire/present cadence and adds an explicit adapter only for VRR. Fixed refresh retains the 3.3 ordered generated-image acquire, including MAKO Decky's 50 ms application-present ceiling, so ordinary FIFO image release on Steam Deck and other non-VRR devices is not misclassified as failed generated load. Requested or active VRR uses the same finite ceiling because lower-image release is not locked to a fixed output-period ladder. A shorter explicit ceiling always wins, and the boundary cannot recursively expand when MAKO's own wait reduces observed source FPS. If VRR transport still cannot supply the next image, MAKO stops that generated delivery, drains the already scheduled backend timeline, preserves the real frame, and moves following frames to zero-wait admission before backend work. Generated delivery resumes as soon as a complete batch is available, but zero-wait preflight remains armed until consecutive complete batches account for one returned-image turnover; a single available frame cannot repeatedly re-expose gameplay to the bounded wait. The turnover proof derives only from the lower image count and generated batch size, so it does not change across 60, 90, 120, or higher refresh rates. One VRR hold therefore cannot become a long scheduler backoff or a repeated bounded stall. Headroom-tight batches remain zero-wait preflight so they cannot reserve work they cannot deliver. Fixed keeps sequential ordered delivery because its multiplier is an explicit workload selection. When the returned pool fits that Fixed batch beyond the application's minimum, no additional relief image is required; without relief, acquisition has a shared ceiling of 50 ms per application present, reduced by any shorter configured ceiling. The experimental Gamescope HDR bridge remains nonblocking.
