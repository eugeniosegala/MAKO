...
The bounded managed scaling exception is selected before process startup. An eligible native host profile with Scaling enabled stages the exact `VK_LAYER_MAKO_render -> VK_LAYER_FROG_gamescope_wsi_x86_64 -> VK_LAYER_MAKO_spatial_scaling` chain. The upper MAKO role owns Frame Generation and relays only the lower role's fixed-surface source capability back to the game; because Gamescope WSI may expose different surface handles on each side, that immutable capability result crosses the DSO boundary through a same-thread, one-shot query relay rather than an upper-to-lower surface-handle lookup. It forwards source-sized swapchain creation unchanged and never allocates spatial resources. The lower role retains exact lower-surface matching for swapchain creation and alone owns spatial reconstruction. A selected MangoHud or vkBasalt layer may follow the spatial role. Profiles without Scaling retain the isolated top-only MAKO contract unless the explicit WSI compatibility option is selected. Layer membership itself is not live.
