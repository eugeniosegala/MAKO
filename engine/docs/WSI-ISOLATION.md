...
Game-owned resolution changes negotiate KHR swapchain maintenance1, or its EXT predecessor, only when the device exposes both extension and feature. MAKO preserves upstream present-fence ownership or attaches preallocated per-image fences. A settled Scale Factor change can use that proof to return one out-of-date result only after the lower present succeeded; MAKO never destroys the application swapchain itself. Once the game destroys it, MAKO removes the context from live updates immediately but retains the lower swapchain until a same-surface replacement present, 50 ms compositor grace, and all fences complete. When destroy-before-create produces a null lower `oldSwapchain`, MAKO drains and destroys the exact retained same-device, same-surface lower object before forwarding a null-old replacement create; it never resurrects that handle through Gamescope WSI. Because that safe destroy-before-create path cannot preserve explicit lower-driver replacement lineage, a scaled null-old replacement presents its first acquired application image directly before any private spatial submission, then resumes scaled real frames and the existing Frame Generation stabilization policy. Explicit-old replacements and cold creates do not take this one-frame WSI prime. Surface destruction is the terminal same-surface boundary and drains the same proof before forwarding. No worker thread, steady-state allocation, or guessed completion is introduced. [Runtime configuration transitions](RUNTIME-TRANSITIONS.md) owns the full lifecycle contract; Gym's Gamescope and Proton lanes prove it end to end.

Frame Generation normally preserves the application's queue-depth baseline and reserves one additional WSI image for each output in the largest generated batch. Ordered FG-only also preserves the release-3.0 FIFO relief image, so a normal three-image 3× request becomes six images and retains the established synchronous presentation path and throughput. A profile with combined scaling configured omits only that relief image at create time: its equivalent lower replacement requests five images because some engines reject or repeatedly churn the six-image replacement topology. Basing this split on the configured scaling lane, rather than a lower-layer activation relay that arrives only after driver creation, keeps the WSI request deterministic; if scaling later fails closed, Frame Generation retains the same bounded combined-path transport for that context. When a combined or surface-clamped pool has no additional spare, MAKO admits its synthetic images nonblockingly before backend work; compositor pressure may skip a generated output but cannot hold the application's real frame behind a blocking acquire. A partial Adaptive admission is durable delivery evidence for that exact WSI context: MAKO rebuilds the scheduler at the admitted lower generated capacity and retains that ceiling until game-owned recreation, preventing its base limiter and higher-multiplier probes from repeatedly pacing the game for unavailable outputs. A fully admitted batch retains the configured maximum, and Fixed mode keeps its explicit multiplier and existing admission behavior. Driver-provided spare images retain the established synchronous ordered path. The surface maximum still takes precedence. If the lower WSI rejects the generated-output reservation with an initialization or memory error, MAKO retries creation once at the application's original minimum.

**Game Swapchain Images (Restart)** is the explicit per-profile escape hatch for games that cannot start with the normal generated-output reservation. When `swapchain_image_count_compatibility = true`, MAKO preserves the application's exact requested minimum from the first relevant swapchain creation and for every later replacement while retaining required transfer usage and the selected ordered FIFO transport. The option is off by default, applies to FG-only and combined FG-plus-scaling profiles, and does not hard-code a three-image pool. With no reserved WSI headroom, native-first nonblocking admission may skip generated outputs when the compositor has no spare image, so enable it only for an affected game. The setting is frozen when the process starts and requires a full game restart to change. MAKO performs no executable, Proton-version, surface-lifetime, present-count, or replacement-pattern detection to activate it; startup diagnostics record the configured policy and requested/provisioned counts directly.
