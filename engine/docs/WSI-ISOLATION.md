...
Frame Generation preserves its established healthy queue-depth contract: MAKO adds one WSI image for the intercepted real frame plus one for each generated output in the largest provisioned batch to the application's requested minimum. This leaves the game's normal in-flight ownership intact while the ordered generated/real sequence is queued, and the surface maximum still takes precedence. If the lower WSI rejects only that inflated minimum with an initialization or memory error, MAKO retries creation once at the application's original minimum. A session-local circuit breaker also selects the application minimum only for the narrow startup lifecycle in which an inflated zero-return probe is destroyed within two seconds of completing context construction, then another inflated surface returns only a small startup burst of two through eight application presents and is destroyed within ten seconds, and the latter is immediately replaced by the same application extent and image-count request. MAKO samples the context's existing completed-frame count only when the swapchain is destroyed, so slower devices have time to complete the startup handoff while a normally running successor is excluded without adding a steady-presentation counter, timestamp, lookup, or timed probe. A healthy, high-frame-count, or long-lived successor clears the pending signature, a different replacement does not match it, and an isolated short-lived surface cannot seed it. The fallback remains active only for that exact extent and requested/provisioned image-count signature for the rest of the game process; another resolution or image-count policy retains normal provisioning. Healthy games therefore keep the normal provisioned depth, while engines that demonstrate this repeated same-geometry startup handoff rely on bounded acquire recovery with their original pool. Startup diagnostics record the requested minimum, normal provisioned minimum, bounded returned-present evidence, active lifetime, candidate geometry, create retry, compatibility evidence, and any activated fallback without consulting a title or executable list.
