...
Manifest order is decisive for the verified spatial-scaling compatibility lane. MAKO Gym's release-only `gamescope-e2e` suite automatically validates the installed 64-bit manifests and libraries, constructs the positive and deliberately reversed layer paths, and runs both through a real headless Gamescope compositor. With Gamescope WSI before MAKO, MAKO observes variable `currentExtent=UINT32_MAX` capabilities and safely activates a 640×360 source to 960×540 presentation split through its existing variable-surface policy. The reverse directory order produces `Application -> MAKO -> Gamescope WSI`; MAKO observes a fixed 640×360 surface and correctly remains inactive rather than forcing a hidden split. The suite also proves Fixed and Adaptive generated delivery, live Native/MAKO/LS1 transitions, FG Off/On without FG-only recreation, maintenance1-fenced retirement, and natural replacement. This evidence supports only the narrow, process-start SDR lane: managed launch may stage WSI before MAKO when the profile starts with Scaling Engine enabled, with an optional post-process manifest directory after MAKO. Changing Scaling Engine inside a running process cannot insert or remove WSI; after the process starts with the ordered lane provisioned, Native/scaler method, factor, and sharpness changes can use the variable-surface live recreation path. Native logs `inactive_reason=native-passthrough` and performs no spatial work. If MAKO instead observes a fixed native extent and the application or upper layer requests that native extent, the policy reports `inactive_reason=application-extent-override-no-source-presentation-split` and remains inactive.
...
This order also defines resolution-change ownership. Gamescope WSI enables swapchain-maintenance1 before chaining device creation into MAKO. For a direct Scaling Engine process where an upper layer has not already enabled it, MAKO requests the KHR or EXT contract itself only after the physical device reports both the matching extension and feature. A Scaling Engine context can then preallocate one lower present fence per image without steady-state allocation. When Gamescope already supplies a non-null `VkSwapchainPresentFenceInfoKHR` on the final lower present, MAKO preserves the structure and ownership unchanged and uses that fence only as the specification-defined proof that the one-shot recreation request followed a protected present. Otherwise MAKO attaches its own per-image fence. When the application destroys an old swapchain, MAKO removes the old context from live configuration immediately but delays the lower `vkDestroySwapchainKHR` until a replacement presentation on the exact same creating surface has passed through the chain, a 50 ms compositor grace has elapsed, and all MAKO-owned maintenance fences have signaled. A present on another window of the same device cannot release it. In the managed Gamescope lane this prevents lower WSI buffer destruction from racing Gamescope's outstanding texture references without borrowing, resetting, or destroying Gamescope's fence. Without negotiated maintenance1, after fence-allocation failure, or with an invalid upstream fence entry, MAKO leaves live resource recreation disabled and uses the natural owner contract rather than guessing completion from a successful `vkQueuePresentKHR` return.
...
- the focused MAKO Gym `gamescope-e2e` rows for layer order, live transition, retirement, and recreation changes, with all seven required before release;
