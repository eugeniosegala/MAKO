...
Frame Generation requires one WSI image for the application's real frame plus one for each generated output in the largest provisioned batch. MAKO therefore raises `minImageCount` only when that batch total exceeds the application's requested minimum; it does not add the two counts. Temporary contention from additional application-held images follows the bounded generated-image acquire recovery and retains native presentation instead of permanently inflating the app-visible swapchain. Startup diagnostics record the requested minimum, forwarded minimum, and actual returned image count so compatibility failures can be distinguished from scheduling pressure without a title-specific exception.
