...
This order also defines resolution-change ownership. Gamescope WSI enables swapchain-maintenance1 before chaining device creation into MAKO. For a direct scaling process where an upper layer has not already enabled it, MAKO requests the KHR or EXT contract itself only after the physical device reports both the matching extension and feature. A scaling context can then preallocate one lower present fence per image without steady-state allocation. When Gamescope already supplies a non-null `VkSwapchainPresentFenceInfoKHR`, MAKO preserves the structure and ownership unchanged; otherwise MAKO attaches its own per-image fence. When the application naturally destroys an old swapchain, MAKO removes the old context from live configuration immediately but delays the lower `vkDestroySwapchainKHR` until a replacement presentation on the exact same creating surface has passed through the chain, a 50 ms compositor grace has elapsed, and all MAKO-owned maintenance fences have signaled. A present on another window of the same device cannot release it. In the managed Gamescope lane this prevents lower WSI buffer destruction from racing Gamescope's outstanding texture references without borrowing, resetting, or destroying Gamescope's fence. Private spatial model changes never enter this application-visible lifetime path. Without negotiated maintenance1, after fence-allocation failure, or with an invalid upstream fence entry, MAKO uses the natural owner contract rather than guessing completion from a successful `vkQueuePresentKHR` return.
