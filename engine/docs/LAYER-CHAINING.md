...
MAKO Decky exposes mutually exclusive **Enable MangoHud** and **Enable vkBasalt** controls under **External Tools**. They share the same per-profile selector with **Experimental Gamescope WSI**, so only one optional Vulkan layer can be active. This is the recommended way to use either external tool because MAKO generates the guarded activation and layer path automatically.
