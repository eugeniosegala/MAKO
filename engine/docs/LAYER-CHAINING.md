...
This guide documents deliberate exceptions to MAKO Renderer's managed implicit-layer isolation and the separate boundary for game-local integrations. [WSI isolation](WSI-ISOLATION.md) remains the architectural source of truth: profiles without Scaling Engine, explicit Gamescope WSI compatibility, or a post-process tool use the isolated default, which gives MAKO one deterministic presentation owner and exposes only MAKO's private Vulkan manifests. A native profile launched with Scaling Engine enabled uses the guarded Gamescope WSI exception described below even when its live method is Native; this does not broaden discovery to unrelated host layers.
...
The explicit compatibility toggle is off by default and stored per profile independently from the post-process selector. Scaling Engine automatically satisfies and locks the same WSI requirement for that profile. In either case the wrapper keeps HDR disabled, verifies that MAKO Renderer installation staged the expected 64-bit host Gamescope WSI manifest, places that managed compatibility directory before MAKO's private manifest directory, enables Gamescope WSI, and retains the known Mesa and competing-frame-generation guards. The directory order is part of the contract: it yields `Application -> Gamescope WSI -> MAKO`, allowing MAKO to receive WSI's internal variable Wayland surface and enlarge the lower presentation extent. If MangoHud or vkBasalt is selected, the installer validates and copies only that tool's exact available 64-bit and 32-bit identities into its own MAKO-managed directory, which the wrapper appends after MAKO to yield `Application -> Gamescope WSI -> MAKO -> selected post-process tool -> Vulkan driver` for a compatible process architecture. MAKO never admits the host's complete implicit-layer directory. The reverse WSI/MAKO order leaves MAKO on the fixed application surface and scaling correctly remains inactive. Every optional layer fails closed when its staged manifest is absent, invalid, or inaccessible, without disabling an independently valid layer choice.
...
MAKO Decky exposes mutually exclusive **Enable MangoHud** and **Enable vkBasalt** controls under **External Tools**. This post-process selection is independent from **Experimental Gamescope WSI** and Scaling Engine, so either tool can run after MAKO while WSI runs before it. This is the recommended way to use either external tool because MAKO generates the guarded activation and ordered layer path automatically.
...
| Gamescope WSI | Excluded from the isolated managed SDR chain. MAKO Decky's explicit compatibility toggle or a native profile launched with Scaling Engine enabled can admit one validated 64-bit host manifest before MAKO while HDR remains disabled. It is independent from the MangoHud/vkBasalt post-process selector. The exact native order and a headless scaling-plus-Fixed probe are verified; real-game pacing, Flatpak, 32-bit presentation, HDR, and broader platform support remain outside that evidence. |
...
- Gamescope WSI appears only when its per-profile compatibility toggle is selected or the native profile started with Scaling Engine enabled; its identity precedes MAKO whether or not one post-process tool is selected, while Mesa device-selection, Mesa anti-lag, competing frame-generation, and unrelated layer identities stay out of the game chain;
