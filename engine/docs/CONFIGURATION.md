...
- **`scaling_enabled`**: Enables scaling independently from frame generation. Enable it before starting the game; when it is off, scaling is fully disabled. It is process-start state, so restart the game after changing it. MAKO Decky uses it to stage the guarded frame-generation-role → Gamescope WSI → spatial-scaling-role chain. On a fixed Gamescope surface, an active scaler advertises a lower source extent and preserves the compositor-owned presentation extent. On a variable desktop surface, the game's requested extent remains the source and MAKO enlarges the lower presentation extent. If a Wayland compositor echoes that enlarged extent back during recreation, MAKO keeps the recreated swapchain native-sized rather than recursively enlarging it; use a fixed Gamescope/X11 path for stable scaled fullscreen sessions. Only validated SDR formats and conventional opaque, single-image-array-layer, unprotected, non-shared-present swapchains are supported. All Gamescope scaling requires HDR exposure to be disabled. Default: `false`.
- **`scaling_method`**: Selects `native`, `mako`, `ls1`, or `ls1-performance`. The UI names are **Native Resolution**, **MAKO Scaler**, **LS1 Quality**, and **LS1 Performance**. `native` is a fast model-free linear reconstruction baseline that retains the same source/presentation split as every model. `mako` is the open single-pass Catmull-Rom, anti-ringing, and sharpening method. `ls1` uses the full multi-pass Lossless Scaling neural network; `ls1-performance` uses its lower-cost network. Both LS1 choices require the user's licensed DLL plus `libvkd3d-shader.so.1` for process-local translation and support MAKO's standard and high-precision SDR transports while keeping the learned graph in its native 8-bit resources. Any LS1 initialization failure is logged and uses `mako` for that swapchain. You can change the model while the game is running; compatible method changes rebuild MAKO's private scaler without recreating the game swapchain while scaling remains provisioned. Default: `native`.
...
- **`scaling_sharpness`**: Normalized `0.0` to `1.0` multiplier for MAKO's bounded local sharpening at its static 2x baseline, or nearest selection among LS1's five learned model variants. Default: `0.5`.
...
- **`flow_scale`**: Frame Generation motion-vector resolution from 0.25 to 1.0. Lower is faster; higher favours image quality. It does not affect spatial scaling. A running context retains its actual flow resources until the game's next natural swapchain recreation or restart. Default: `1.0`.
- **`performance_mode`**: Stable compatibility property presented as **Lighter FG Model**. It uses a lighter model for lower GPU cost and more artifacts. A running context retains its actual model until the game's next natural swapchain recreation or restart. Default: `false`.
...
Frame Generation turns off and back on live in a matched process whose startup interop, backend, and private resources succeeded. Its refresh-rate threshold, Fixed/Adaptive mode, multiplier within reserved capacity, Adaptive target/ceiling, Smooth Cadence, Dynamic Cadence Recovery, and probe interval also apply while the game is running, including inside an already-active Ultra Performance session. Interval-only changes reschedule the next probe without resetting the validated scheduling policy. The threshold guard and Fixed recovery also react live when Gamescope reports a refresh-rate change. Scaling enablement remains process-static because the managed launch chain cannot gain or lose Gamescope WSI after Vulkan instance creation. With scaling already provisioned, Native Resolution/MAKO Scaler/LS1 method changes rebuild only MAKO's private spatial context at the next present. Sharpness does the same after a 500 ms quiet period. These transitions preserve the application swapchain and Gamescope WSI object, warm frame-generation history, retain the old scaler on failure, and log requested plus active methods. Scale Factor changes the extent contract and waits for the game's next natural resolution change or restart. Flow Scale, Lighter FG Model, pacing shape, and generated-capacity growth also wait for natural game-owned recreation; their pending state does not block unrelated live-safe settings. Natural replacement still uses maintenance1 fences, same-surface lower-handle handoff, compositor grace, and surface-terminal retirement, but MAKO does not synthesize an out-of-date result for a spatial model change in the managed Gamescope lane. Continue to restart after changing Scaling, the DLL path, FP16 policy, GPU, HDR-related settings, toggling Ultra Performance, or another setting that requires process/device construction. The complete lifecycle, partial-application, reset, diagnostics, and extension contract is documented in [Runtime configuration transitions](RUNTIME-TRANSITIONS.md).
