...
Frame Generation turns off and back on live in a matched process whose startup interop, backend, and private resources succeeded. Its refresh-rate threshold, Fixed/Adaptive mode, multiplier, Adaptive target/ceiling, Smooth Cadence, Dynamic Cadence Recovery, and its probe interval also apply while the game is running, including inside an already-active Ultra Performance session. Interval-only changes reschedule the next probe without resetting the validated scheduling policy. The threshold guard and Fixed recovery also react live when Gamescope reports a refresh-rate change. Scaling Engine enablement remains process-static because the managed launch chain cannot gain or lose Gamescope WSI after Vulkan instance creation. With the engine already provisioned, Native Resolution/MAKO Scaler/LS1 method, factor, and sharpness changes use the recreation path. A pending resource-shape or process-restart edit does not block unrelated live-safe fields: the Renderer applies the safe subset and retains the remaining request for its documented boundary. Discrete method changes use the next retirement-protected lower present. A multiplier or mode change that needs more generated-frame capacity, spatial factor/sharpness, Flow Scale, and Lighter FG Model changes retain a 500 ms quiet period, then MAKO converts one successful retirement-protected lower result into one `VK_ERROR_OUT_OF_DATE_KHR`. MAKO preserves an upstream Gamescope present fence as the completion proof for the final lower present, or attaches its own preallocated maintenance1 fence when no upstream owner exists. The game remains responsible for requesting swapchain destruction and recreation with the latest settings; the managed Gamescope lane defers actual lower WSI destruction through a later replacement present, a short compositor grace, and completion of every MAKO-owned associated present fence. A brief flicker is normal. If maintenance1 is unavailable, the fence is invalid, or fence allocation fails, MAKO waits for natural recreation instead of guessing. Pacing shape waits for a natural game-owned swapchain recreation. A process with retained frame-generation resources rebuilds its private LSFG context with the selected output capacity, flow resolution, and model during recreation. If startup provisioning failed or a title ignores the one-shot signal, restart it as the compatibility fallback. Continue to restart after changing Scaling Engine, the DLL path, FP16 policy, GPU, HDR-related settings, toggling Ultra Performance, or another setting that requires process/device construction. The complete lifecycle, partial-application, reset, diagnostics, and extension contract is documented in [Runtime configuration transitions](RUNTIME-TRANSITIONS.md).
