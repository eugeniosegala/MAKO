...
Frame Generation, its refresh-rate threshold, Fixed/Adaptive mode, multiplier within existing capacity, Adaptive target/ceiling, Smooth Cadence, Dynamic Cadence Recovery, and its probe interval can usually apply while the game is running. Interval-only changes reschedule the next probe without resetting the validated scheduling policy. The threshold guard and Fixed recovery also react live when Gamescope reports a refresh-rate change. A pending resource-shape or process-restart edit does not block unrelated live-safe fields: the Renderer applies the safe subset and retains the remaining request for its documented boundary. Flow Scale, Lighter FG Model, pacing shape, and generated-capacity growth can apply after a natural swapchain recreation; DLL path, FP16 policy, GPU selection, Ultra Performance, HDR-related settings, and launch-environment settings require a game restart. Normal mode retains idle private resources while Frame Generation is off so it can be enabled live, but the native presentation path performs no copies, model scheduling, private fences, or generated-image work. Ultra Performance freezes every user-profile change for the running process and skips configuration-file polling entirely; when Frame Generation is explicitly off, it also omits private generation resources because no live enable is possible. Gamescope refresh and HDR feedback continue updating because they are runtime safety inputs rather than user toggles. The complete lifecycle, partial-application, reset, diagnostics, and extension contract is documented in [Runtime configuration transitions](RUNTIME-TRANSITIONS.md).
