...
- **`dll`**: Optional full path to `Lossless.dll`, which supplies the LSFG frame-generation and LS1 scaling model shaders. Leave it unset to use MAKO's normal Steam-library discovery. Native Resolution and MAKO Scaler do not use the licensed spatial models. Every matched process reserves the application-device interop needed for live Frame Generation, while a missing backend keeps generation unavailable and leaves an independent spatial scaler usable.
...
- **`swapchain_image_count_compatibility`**: Preserves the application's exact requested swapchain image minimum instead of reserving generated-output headroom. This can let affected games start across Proton/runtime variants because it applies from the first swapchain create without lifecycle or executable detection. Required transfer usage and ordered FIFO transport remain active, but generated frames may be skipped when the compositor has no spare image. It is per profile, off by default, applies to FG-only and combined FG-plus-scaling, and requires a full game restart to change. Default: `false`.
...
- **`scaling_factor`**: Source-to-presentation ratio from `1.0` to `2.0`; `1.0` performs no scaling work. Fixed surfaces derive an even source extent from the compositor-owned presentation extent. Variable surfaces enlarge the application's request within surface and device-memory limits; a positively identified Gamescope output also bounds the effective factor so ordinary scaling does not render supersampled pixels that the compositor immediately discards. A source that already fills either output dimension stays native. Default: `1.5`.
- **`scaling_supersampling`**: Explicitly allows a variable Gamescope surface to exceed its proven output target when `scaling_factor` requests a larger presentation image. Vulkan surface limits and MAKO's deterministic device-memory envelope remain mandatory. The default `false` keeps ordinary scaling at the real device-output ceiling and avoids discarded offscreen work. Fixed surfaces already have a compositor-owned extent, and direct non-Gamescope variable surfaces have no inferred monitor target, so this option does not change their geometry. Default: `false`.
- **`scaling_sharpness`**: Normalized `0.0` to `1.0` multiplier for MAKO's bounded local sharpening at its static 3x baseline, or nearest selection among LS1's five learned model variants. Default: `0.8`.
...
- **`adaptive_auto_base_fps_cap`**: In Adaptive mode, starts with a half-target real-frame cap for an even 2x baseline. When Smooth Cadence, ordered Gamescope SDR, and a target-matching refresh are all active, a proven 3x-5x scheduler level may qualify the exact `target_fps / multiplier` cap for a steadier integer cadence. If ordered SDR proves that combined rendering has collapsed the capped 2x path well below target, MAKO releases only this automatic cap for the current scheduler and returns to Fractional recovery; manual and Fixed caps remain authoritative. Ramp, recovery, or lost qualification otherwise restores the conservative half-target cap, while a new swapchain or scheduler-policy change clears the collapse proof. It can trade real-frame headroom and responsiveness for steadier output, with potentially more input lag and ghosting. Default: `false`.
...
- **`ultra_performance`**: Restart-bound preset that forces an effective Flow Scale of 0.75, the lighter frame-generation model, FP16 permission when supported, and active-policy-only generated-output allocation. It may improve GPU-limited performance by up to 30% but increases artifacts. Compatible runtime controls remain available; later capacity growth uses the normal private FG replacement boundary, but toggling the preset still requires restart because FP16 and the resource policy are process-static. Both MAKO Decky and `mako-ui` restore their shared 0.90/false/true defaults when disabling the preset. Default: `false`.
- **`flow_scale`**: Frame Generation motion-vector resolution from 0.25 to 1.0. Lower is faster; higher favours image quality. It does not affect spatial scaling. After a 500 ms quiet period, MAKO prepares a replacement private FG context and atomically switches when its old private work is idle. Default: `0.9`.
...
- `MAKO_SCALING_ENABLED`, `MAKO_SCALING_METHOD`, `MAKO_SCALING_FACTOR`, `MAKO_SCALING_SHARPNESS`, `MAKO_SWAPCHAIN_IMAGE_COUNT_COMPATIBILITY`
