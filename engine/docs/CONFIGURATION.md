...
- **`scaling_enabled`**: Enables spatial reconstruction independently from frame generation. On a fixed Gamescope surface, MAKO advertises a lower source extent and preserves the compositor-owned presentation extent. On a variable desktop surface, the game's requested extent remains the source and MAKO enlarges the lower presentation extent. If a Wayland compositor echoes that enlarged extent back during recreation, MAKO keeps the recreated swapchain native-sized rather than recursively enlarging it; use a fixed Gamescope/X11 path for stable scaled fullscreen sessions. Only validated SDR formats and conventional opaque, single-image-array-layer, unprotected, non-shared-present swapchains are supported. All Gamescope scaling requires HDR exposure to be disabled. Default: `false`.
- **`scaling_method`**: Selects `mako`, `ls1`, or `ls1-performance`. `mako` is the open single-pass Catmull-Rom, anti-ringing, and sharpening method. `ls1` uses the full multi-pass Lossless Scaling neural network; `ls1-performance` uses its lower-cost network. Both LS1 choices require the user's licensed DLL plus `libvkd3d-shader.so.1` for process-local translation and support MAKO's standard and high-precision SDR transports while keeping the learned graph in its native 8-bit resources. Any LS1 initialization failure is logged and uses `mako` for that swapchain. Default: `mako`.
...
- `MAKO_SCALING_ENABLED`, `MAKO_SCALING_FACTOR`, `MAKO_SCALING_SHARPNESS`
