...
| `adaptive_stable_cadence` | Boolean | `true` | Allows a delivery-validated constant cadence. In Fixed mode, ordered Gamescope presentation can pace a stable source to the selected refresh/multiplier rung; in Adaptive it retains a validated cadence. This may trade real-frame cadence and latency for smoother output. |
