...
| `frame_generation_enabled` | Boolean | `true` | Live Frame Generation execution state. `false` is the `0x` position in Decky and the desktop GUI: no generation work is submitted, but startup-provisioned resources stay available for a later live enable. Fixed shows a **Fixed Multiplier** control with `0x` and 2x–5x; Adaptive replaces it with **Maximum Adaptive Multiplier** using the same choices. |
...
| `adaptive_stable_cadence` | Boolean | `true` | Allows a delivery-validated constant cadence in Adaptive. Eligible Fixed mode hands pacing to ordered Gamescope FIFO, including under VRR. Steady Adaptive uses FIFO for proven 2x and can test full-batch FIFO on validated 3x–5x rungs under VRR. Fractional Adaptive can hand an already accepted constant 2x–5x cadence to FIFO under VRR when no real-frame cap or Dynamic Cadence Recovery competes; its variable plans retain target-clock pacing. The higher Steady multiplier base-cap ladder still requires fixed-refresh eligibility. FIFO may trade real-frame cadence and latency for smoother output. |
...
The Qt UI's **Shaders** group is per profile and uses the same compact settings as MAKO Decky: activation, an ordered selection of curated effects, CAS or DLS sharpening, sharpening and DLS-denoise strengths, and FXAA or SMAA. Both UIs store the ordered effect IDs in the same `vkbasalt_shader` field of `~/.config/mako-render/profile-wrapper-settings.json` and reference the curated sources under `~/.config/mako-render/vkbasalt/shaders/`, so a profile saved in one UI can be reopened and edited in the other without changing its effect order or shader paths. Close one editor before opening the other; an already-open Qt window does not watch Decky's changes. Steam-linked profiles reuse the app-ID-based config path recorded in `profile-metadata.json`; other saved profiles use the same stable short hash as Decky. The default `mako` profile uses vkBasalt's global config. Packaged assets remain available under `~/.local/share/mako-render/vkbasalt-shaders/` so profiles written by older Qt builds survive Renderer replacement.
