...
- **`scaling_factor`**: Source-to-presentation ratio from `1.0` to `2.0`; `1.0` performs no scaling work. Fixed surfaces derive an even source extent from the compositor-owned presentation extent. Variable surfaces enlarge the application's request within surface and device-memory limits. Default: `1.5`.
...
- **`ultra_performance`**: Restart-bound preset that forces an effective Flow Scale of 0.75, the lighter frame-generation model, FP16 permission when supported, and active-policy-only generated-output allocation. It may improve GPU-limited performance by up to 30% but increases artifacts. Compatible runtime controls remain available; capacity growth uses the normal game-owned recreation boundary. MAKO Decky restores its 0.90/false/true defaults when disabling the preset, while `mako-ui` restores the direct Renderer defaults of 1.00/false/true. Default: `false`.
- **`flow_scale`**: Frame Generation motion-vector resolution from 0.25 to 1.0. Lower is faster; higher favours image quality. It does not affect spatial scaling. The active value changes at the next game-owned recreation. Default: `1.0`.
- **`performance_mode`**: Stable compatibility property presented as **Lighter FG Model**. It uses a lighter model for lower GPU cost and more artifacts and changes at the next game-owned recreation. Default: `false`.
...
Frame Generation enablement and scheduling controls apply live when startup provisioning succeeded. With scaling provisioned, method changes rebuild the private scaler at the next present and sharpness changes do so after a 500 ms quiet period. Scale Factor, Flow Scale, Lighter FG Model, and capacity growth apply at game-owned recreation: compatible non-Gamescope maintenance1 contexts request one safely, while managed Gamescope waits for a natural recreation. Pacing shape always waits for natural recreation. Scaling enablement, DLL, FP16, GPU, HDR/WSI policy, and Ultra Performance require restart. Mixed writes still apply their live-safe subset while reporting deferred fields. [Runtime configuration transitions](RUNTIME-TRANSITIONS.md) owns the exact matrix, state-reset rules, and diagnostics.
