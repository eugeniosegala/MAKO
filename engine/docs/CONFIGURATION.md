...
- **`scaling_factor`**: Ratio from each source dimension to each presentation dimension, from `1.0` to `2.0`. A fixed surface derives `source = floor(presentation / factor)` with even rounding; a variable surface derives an aspect-preserving presentation extent from the application's request and clamps it to the surface maximum. `1.0` performs no scaling work. Default: `1.5`.
