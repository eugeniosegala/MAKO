...
| `adaptive_stable_cadence` | Boolean | `true` | Allows a delivery-validated constant cadence in Adaptive. Eligible Fixed mode hands pacing to ordered Gamescope FIFO, including under VRR. Steady Adaptive uses FIFO for proven 2x and can test full-batch FIFO on validated 3x–5x rungs under VRR; the higher multiplier base-cap ladder still requires fixed-refresh eligibility. This may trade real-frame cadence and latency for smoother output. |
