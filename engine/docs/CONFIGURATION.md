...
- **`ultra_performance`**: Restart-only per-profile setting that forces an effective Flow Scale of 0.80, the lighter frame-generation model, and FP16 permission when supported to reduce GPU work. It allocates generated-output resources only for the active Fixed or Adaptive policy and increases visual artifacts. No live user-configuration updates take place while it is enabled, so restart the game after changing settings for them to apply. MAKO Decky stores 0.80/true/true when enabling it and restores its 0.90/false/true defaults when disabling it; `mako-ui` restores the direct Renderer defaults of 1.00/false/true. The Renderer applies the forced effective values defensively even to a manually inconsistent profile. Default: `false`.
