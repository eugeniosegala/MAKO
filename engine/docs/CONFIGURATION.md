...
Configure MAKO Renderer with `mako-ui` or by editing `~/.config/mako-render/conf.toml`. The UI writes the same TOML file. MAKO creates profiles so different games can use different settings. See [Spatial scaling architecture](SCALING.md) for the scaler's pipeline order, surface and format boundaries, resource model, and validation requirements.
...
- **`dll`**: Optional full path to `Lossless.dll`, which supplies the LS1 scaling and LSFG frame-generation model shaders. Leave it unset to use MAKO's normal Steam-library discovery. MAKO scaling-only does not discover the DLL; LS1 scaling-only reads its selected model resources but does not provision LSFG interop or the private frame-generation backend.
...
- **`scaling_enabled`**: Provisions Scaling Engine independently from frame generation. Treat this as process-start state and restart the game after changing it; MAKO Decky uses it to stage the guarded Gamescope WSI-before-MAKO layer chain. On a fixed Gamescope surface, an active scaler advertises a lower source extent and preserves the compositor-owned presentation extent. On a variable desktop surface, the game's requested extent remains the source and MAKO enlarges the lower presentation extent. If a Wayland compositor echoes that enlarged extent back during recreation, MAKO keeps the recreated swapchain native-sized rather than recursively enlarging it; use a fixed Gamescope/X11 path for stable scaled fullscreen sessions. Only validated SDR formats and conventional opaque, single-image-array-layer, unprotected, non-shared-present swapchains are supported. All Gamescope scaling requires HDR exposure to be disabled. Default: `false`.
- **`scaling_method`**: Selects `native`, `mako`, `ls1`, or `ls1-performance`. `native` retains process-start engine provisioning but performs no MAKO spatial allocation or dispatch. `mako` is the open single-pass Catmull-Rom, anti-ringing, and sharpening method. `ls1` uses the full multi-pass Lossless Scaling neural network; `ls1-performance` uses its lower-cost network. Both LS1 choices require the user's licensed DLL plus `libvkd3d-shader.so.1` for process-local translation and support MAKO's standard and high-precision SDR transports while keeping the learned graph in its native 8-bit resources. Any LS1 initialization failure is logged and uses `mako` for that swapchain. Method changes use game-owned swapchain recreation while Scaling Engine remains provisioned. Default: `mako`.
- **`scaling_factor`**: Ratio from each source dimension to each presentation dimension, from `1.0` to `2.0`. A fixed surface derives `source = floor(presentation / factor)` with even rounding; a variable surface derives an aspect-preserving presentation extent from the application's request and clamps it to the surface maximum. Before allocation, a variable candidate must also fit the device-local-memory presentation envelope; an exact candidate beyond that envelope remains native instead of allocating a pathological lower swapchain. `1.0` performs no scaling work. Default: `1.5`.
- **`scaling_sharpness`**: Normalized `0.0` to `1.0` strength for MAKO's bounded local sharpening, or nearest selection among LS1's five learned model variants. Default: `0.5`.
- **`multiplier`**: Supported fixed multipliers are 2x, 3x, and 4x; do not configure larger values. The direct Renderer default is `2`.
- **`frame_generation_enabled`**: On/off switch. `false` presents real frames, reconstructed first when scaling is active, while keeping the layer loaded. A process that starts with `false` does not negotiate external-memory, external-semaphore, or timeline-semaphore interop or allocate frame-generation resources, so changing it to `true` requires a complete game/process restart. LS1 may still read `Lossless.dll` for spatial shaders without creating the private LSFG backend. A process that started with Frame Generation enabled retains its LSFG resources and can turn generation off and back on live. Default: `true`.
...
- **`adaptive_auto_base_fps_cap`**: In Adaptive mode, caps the real frame rate to half of `target_fps` for an even 2x baseline. It can trade real-frame headroom and responsiveness for steadier output, with potentially more input lag and ghosting. Default: `false`.
...
- **`adaptive_stable_cadence`**: Prefers a validated constant interpolation cadence when it is sustainable. On ordered Gamescope SDR with display refresh matching the Adaptive target, it may test a nearby 2x cadence and retain it only when output settles within 98–102% of target. A validated Steady 2x path lets ordered FIFO own pacing instead of retaining a redundant explicit cap; any qualification or recovery failure restores the cap immediately and backs off before retrying. This can look smoother but may lower real-frame cadence and increase input lag and ghosting. Default: `false`.
...
- **`ultra_performance`**: Restart-bound per-profile setting that may improve frame-generation performance by up to 30% in favourable GPU-limited scenarios by forcing an effective Flow Scale of 0.75, the lighter frame-generation model, FP16 permission when supported, and active-policy-only generated-output allocation. It increases visual artifacts and is primarily intended for low-power devices such as Steam Deck. Turning the preset on or off requires a game restart because its FP16 and allocation policy are selected at process startup. Other compatible profile controls remain live inside a process that started with Ultra Performance active; mode or multiplier changes grow active-policy capacity through the normal game-owned swapchain recreation when required. Gamescope refresh and HDR feedback, presentation safety, and recovery remain active. MAKO Decky stores 0.75/true/true when enabling it and restores its 0.90/false/true defaults when disabling it; `mako-ui` restores the direct Renderer defaults of 1.00/false/true. The Renderer applies the forced effective values defensively even to a manually inconsistent profile. Default: `false`.
- **`flow_scale`**: Motion-vector resolution from 0.25 to 1.0. Lower is faster; higher favours image quality. Changes apply live through the debounced game-owned swapchain recreation described below. Default: `1.0`.
- **`performance_mode`**: Stable compatibility property presented as **Lighter FG Model**. It uses a lighter model for lower GPU cost and more artifacts. Changes apply live through the same swapchain-recreation boundary as Flow Scale and spatial scaling. Default: `false`.
...
Frame Generation can turn off and back on live only when the process started with it enabled and retained its interop/backend resources. Enabling it after a scaling-only process start requires a full game/process restart; swapchain recreation is not sufficient. Its refresh-rate threshold, Fixed/Adaptive mode, multiplier, Adaptive target/ceiling, Smooth Cadence, Dynamic Cadence Recovery, and its probe interval can otherwise apply while the game is running, including inside an already-active Ultra Performance session. Interval-only changes reschedule the next probe without resetting the validated scheduling policy. The threshold guard and Fixed recovery also react live when Gamescope reports a refresh-rate change. Scaling Engine enablement is likewise process-static because the managed launch chain cannot gain or lose Gamescope WSI after Vulkan instance creation. With the engine already provisioned, Native/scaler method, factor, and sharpness changes use the recreation path. A pending resource-shape or process-restart edit does not block unrelated live-safe fields: the Renderer applies the safe subset and retains the remaining request for its documented boundary. Discrete Native/scaler method changes use the next successful lower present. A multiplier or mode change that needs more generated-frame capacity, spatial factor/sharpness, Flow Scale, and Lighter FG Model changes retain a 500 ms quiet period, then MAKO forwards one final frame so the lower WSI consumes the application's wait semaphores and converts that successful result into one `VK_ERROR_OUT_OF_DATE_KHR`. The game remains responsible for destroying and recreating its swapchain with the latest settings; a brief flicker is normal. Pacing shape waits for a natural game-owned swapchain recreation. A process with retained frame-generation resources rebuilds its private LSFG context with the selected output capacity, flow resolution, and model during recreation. If a title ignores the one-shot signal, restart it as the compatibility fallback. Continue to restart after changing Scaling Engine, the DLL path, FP16 policy, GPU, HDR-related settings, toggling Ultra Performance, or another setting that requires process/device construction. The complete lifecycle, partial-application, reset, diagnostics, and extension contract is documented in [Runtime configuration transitions](RUNTIME-TRANSITIONS.md).
