...
| `adaptive_stable_cadence` | Boolean | `true` | Allows a delivery-validated constant cadence in Adaptive. Eligible Fixed mode hands pacing to ordered Gamescope FIFO, including under VRR. A proven Steady Adaptive 2x cadence may also use FIFO under VRR; its higher multiplier cap ladder still requires fixed-refresh eligibility. This may trade real-frame cadence and latency for smoother output. |
