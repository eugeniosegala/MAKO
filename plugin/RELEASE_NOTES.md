...
- **SteamOS presentation is smoother and easier to monitor:** MAKO works more reliably alongside Gamescope, Steam, Game Mode, and overlays without competing with Gamescope's Vulkan WSI policy. Private manifests prevent unintended activation, while an off-by-default per-profile compatibility control can reintroduce Gamescope WSI for games affected by coloured or pixelated motion artifacts. The same mutually exclusive selection admits either MangoHud or experimental vkBasalt through the guarded 64-bit SteamOS path; MangoHud can report final generated FPS and keeps using the user's existing overlay configuration.
...
- **A clearer interface with safer maintenance:** Game Mode gains cleaner spacing, hierarchy, compatibility controls, an inline timed post-install restart recommendation that remains visible before the installed interface appears, and complete Simplified Chinese support. Upgrades now rebuild wrappers from canonical settings and atomically replace only MAKO-managed installation files and generated wrappers without following stale symlinks or rewriting them in place; read-only user configuration is preserved and reported clearly. Self-contained tester ZIPs fail packaging if Decky would attempt an invalid payload download, diagnostics remain focused, and expanded real-world testing covers SteamOS, AMD FP32/FP16, packaging, frontend, backend, and Vulkan-loader boundaries.
