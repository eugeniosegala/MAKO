...
- **Upgrade-safe profiles:** Unknown or retired profile values cannot become engine state or wrapper exports and are removed on the next canonical save. Meaningful legacy settings use explicit migrations, preventing stale options from silently reactivating.
- **Clean Armada safety boundary:** Renderer staging stays inside MAKO's user-owned data directory and uses deterministic permissions, avoiding Armada/FEX's translated `/tmp` boundary. The package declares its x86_64 host requirement and refuses incompatible native AArch64 installation. An older wrapper is converted to an early, configuration-free passthrough that preserves `armada-game-launch` exactly once, and MAKO-owned Flatpak activation from older builds is removed. Native Armada rendering remains deliberately disabled until a source-built payload passes the documented FEX, Turnip, image-quality, and game matrix.
