...
MAKO Decky’s Abyss Ascending release adds a provisioned Scaling Engine with Native passthrough, independent LS1 neural scaling, and a built-in open MAKO scaler alongside a major Adaptive Frame Generation upgrade for Game Mode. Scaling can reconstruct a lower-resolution game frame for direct presentation or run once before Fixed or Adaptive Frame Generation, while Fractional Adaptive handles uneven frame delivery more gracefully and Smooth Cadence only locks into a steady path when the game can truly sustain it. LS1 and LSFG use the user's licensed `Lossless.dll`; Native and the open MAKO scaler do not, and a scaling-only process still omits all LSFG interop.

- **Scaling is now a first-class per-game mode:** The new Spatial Scaling section provides a restart-bound Scaling Engine switch plus Native, LS1 Quality, LS1 Performance, and open MAKO method choices. Once a game starts with the engine provisioned, method, 1.0x–2.0x factor, and sharpening/model-variant changes use one debounced game-owned swapchain recreation. Native keeps the presentation lane available without allocating or dispatching a scaler; LS1 automatically uses MAKO and logs why if its licensed resources or translator are unavailable. Scaling can run with Frame Generation on or off and retains its tuning when the engine is disabled. Enabling Frame Generation after a scaling-only process start still requires a restart.
