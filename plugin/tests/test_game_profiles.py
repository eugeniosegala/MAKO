...
class GameProfileTests(unittest.TestCase):
    ...
    def test_named_profile_can_be_loaded_without_activating_it(self):
        ...
        self.assertEqual(
            loaded["vkbasalt_config_path"],
            str(self.service._vkbasalt_config_path(profile_name)),
        )
        self.assertEqual(
    ...
    def test_vkbasalt_config_is_selected_automatically_and_merged_per_profile(self):
        game = dict(ConfigurationManager.get_defaults())
        game["active_in"] = "CoolGame.exe"
        profile_data = ProfileData(
            current_profile="mako",
        ...
        legacy_default_path.write_text(
            "# Generated by MAKO Decky. Changes will be replaced.\n"
            "effects = cas\n"
        ...
        self.service.vkbasalt_layer_dir = (
            self.service.config_dir.parent / "vkbasalt-layer"
        )
        self.service.vkbasalt_layer_dir.mkdir(parents=True)
        (
            self.service.vkbasalt_layer_dir /
            configuration_module.VKBASALT_MANIFEST_FILENAME_64
        ).write_text("{}", encoding="utf-8")
        ...
        self.assertIn(
            "casSharpness = 0.25",
            Path(unrelated["VKBASALT_CONFIG"]).read_text(encoding="utf-8"),
        )
        self.assertEqual(
            Path(unrelated["VKBASALT_CONFIG"]),
        ...
        game_config_path = Path(matched["VKBASALT_CONFIG"])
        game_config_path.write_text(
            "# Keep this advanced configuration.\n"
            "effects = deband:fxaa:dls # preserve order\n"
            "debandIterations = 2\n"
            "dlsSharpness = 0.10\n"
            "dlsDenoise = 0.11\n",
            encoding="utf-8",
        ...
        stale_path = self.service.vkbasalt_profile_config_dir / (
            "0" * 24 + ".conf"
        )
        stale_path.write_text("effects = cas\n", encoding="utf-8")
        self.service._write_wrapper_profile_settings({
            "mako": {
                "external_vulkan_layer": "vkbasalt",
            },
    ...
    def test_default_vkbasalt_uses_global_file_and_ignores_caller_override(self):
        ...
        values = self._run_wrapper("", {
            "VKBASALT_CONFIG_FILE": "/tmp/user-selected-vkbasalt.conf",
        })
        self.assertEqual(
            values["VKBASALT_CONFIG"],
            str(self.service.vkbasalt_global_config_path),
        )
    ...
    def test_deleting_profile_removes_identity_and_wrapper_settings(self):
        ...
        config = self.service.get_profile_config(name)["config"]
        ...
        self.assertFalse(vkbasalt_path.exists())

    def test_renaming_profile_keeps_advanced_vkbasalt_edits(self):
        created = self.service.create_profile("Manual Game")
        old_name = created["profile_name"]
        config = self.service.get_profile_config(old_name)["config"]
        config["external_vulkan_layer"] = "vkbasalt"
        self.assertTrue(
            self.service.update_profile_config(old_name, config)["success"]
        )
        old_path = self.service._vkbasalt_config_path(old_name)
        old_path.write_text(
            old_path.read_text(encoding="utf-8") + "lutFile = custom.cube\n",
            encoding="utf-8",
        )

        renamed = self.service.rename_profile(old_name, "Renamed Game")

        self.assertTrue(renamed["success"])
        new_path = self.service._vkbasalt_config_path(renamed["profile_name"])
        self.assertFalse(old_path.exists())
        self.assertIn("lutFile = custom.cube", new_path.read_text(encoding="utf-8"))
