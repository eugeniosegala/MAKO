...
class GameProfileTests(unittest.TestCase):
    ...
    def test_format_31_wrapper_only_settings_are_imported(self):
        self.service.mako_script_path.write_text(
            "\n".join([
                "#!/bin/bash",
                "# mako-wrapper-format: 31",
        ...
        self.assertTrue(self.service.migrate_wrapper_profile_settings_if_needed())

        payload = json.loads(
            self.service.wrapper_profile_settings_path.read_text(encoding="utf-8")
        )
        self.assertTrue(payload["profiles"]["mako"]["disable_mako"])
        self.assertTrue(payload["profiles"]["mako"]["force_alsa_audio"])

    def _write_stale_multi_profile_wrapper(self, wrapper_format: int) -> None:
        game = dict(ConfigurationManager.get_defaults())
        game["active_in"] = "CoolGame.exe"
        profile_data = ProfileData(
            current_profile="mako",
        ...
        script = self.service._generate_script_content_for_profile(profile_data)
        self.assertIn('case "$mako_wrapper_profile" in', script)
        self.service.mako_script_path.write_text(
            script.replace(
                self.service._WRAPPER_FORMAT_MARKER,
                f"# mako-wrapper-format: {wrapper_format}",
                1,
            ),
            encoding="utf-8",
        )
        self.service.wrapper_profile_settings_path.unlink()

    def test_format_32_wrapper_is_never_reverse_migrated_as_one_profile(self):
        self._write_stale_multi_profile_wrapper(32)

        self.assertFalse(self.service.migrate_wrapper_profile_settings_if_needed())
        self.assertFalse(self.service.wrapper_profile_settings_path.exists())

    def test_released_format_45_wrapper_is_never_reverse_migrated_as_one_profile(self):
        self._write_stale_multi_profile_wrapper(45)

        self.assertFalse(self.service.migrate_wrapper_profile_settings_if_needed())
        self.assertFalse(self.service.wrapper_profile_settings_path.exists())

    def test_malformed_and_future_wrapper_markers_fail_closed(self):
        for marker in (
                "# mako-wrapper-format: invalid",
                "# mako-wrapper-format: 999",
        ):
            with self.subTest(marker=marker):
                ...
                self.assertFalse(
                    self.service.wrapper_profile_settings_path.exists()
                )

    def test_unmarked_legacy_wrapper_only_settings_are_imported(self):
        self.service.mako_script_path.write_text(
            "#!/bin/bash\nexport DISABLE_MAKO=1\n",
            encoding="utf-8",
        )
    ...
    def test_unmarked_multi_profile_wrapper_fails_closed(self):
        self._write_stale_multi_profile_wrapper(32)
        script = self.service.mako_script_path.read_text(encoding="utf-8")
        self.service.mako_script_path.write_text(
            script.replace("# mako-wrapper-format: 32\n", "", 1),
            encoding="utf-8",
        )
