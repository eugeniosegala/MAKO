...
describe("game profile selection", () => {
  ...
  test("changes the selected profile only after the backend accepts it", async () => {
    mocks.setCurrentProfile.mockResolvedValue({ success: false, error: "game still running" });
    const { result } = renderHook(() => useProfileManagement());
    await waitFor(() => expect(result.current.profiles).toEqual(["mako", "elden-ring"]));

    await act(() => result.current.setCurrentProfile("elden-ring"));
    expect(result.current.currentProfile).toBe("mako");
    expect(mocks.showErrorToast).toHaveBeenCalledWith("Failed to switch profile", "game still running");

    mocks.setCurrentProfile.mockResolvedValue({ success: true, profile_name: "elden-ring" });
    await act(() => result.current.setCurrentProfile("elden-ring"));
