...
import { MAKO_INSTALL_COMPLETION_DURATION_MS } from "../components/MakoInstallCountdown";
import t from "../i18n/i18n";

export function useInstallationActions() {
  const [isInstalling, setIsInstalling] = useState<boolean>(false);
  const [isUninstalling, setIsUninstalling] = useState<boolean>(false);
  const [isInstallCompletionVisible, setIsInstallCompletionVisible] = useState<boolean>(false);
