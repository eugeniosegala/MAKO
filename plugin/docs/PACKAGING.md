...
The ZIP keeps the established `Mako/` directory and installs to `~/homebrew/plugins/Mako` so a package replaces earlier versions instead of creating a second case-sensitive directory. This is an internal compatibility slug. The Decky manifest/listing name is **MAKO - Scaling & Frame Generation**, while project documentation, the frontend, and lifecycle logs identify the component as **MAKO Decky**. Local deployments and validated tester packages also accept the previous **MAKO - Frame Generation** manifest name so developer installations update in place.
