...
Local packages embed the Renderer archive under `bundled_renderer` and omit Decky's `remote_binary`, preventing a local ZIP from silently downloading another build. The verifier rejects missing payloads, checksum mismatches, unsafe URLs, or inconsistent metadata. The stable install slug remains `Mako/`; the listing name is **MAKO - Scaling & Frame Generation**, with the previous listing name accepted only for the documented local migration.
