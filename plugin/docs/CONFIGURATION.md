...
A restart-bound or private-resource edit does not block unrelated compatible controls: MAKO applies the safe subset while the remaining boundary stays pending. Renderer-owned requested-versus-applied records remain available to diagnostics. MAKO Decky polls them only while a game is active to expose an unsupported Gamescope WSI surface beside Scaling controls; it does not show transition pop-ups. A saved profile is not itself proof that every field is active.
