...
- **Enable Scaling (Restart):** Adds MAKO frame generation, Gamescope WSI, and MAKO spatial scaling roles at process start. Changing the toggle is saved immediately but cannot change the Vulkan layer chain of a running game. Once enabled at startup, the method remains live. Leave it off when scaling is not needed: scaling is then fully disabled, the path is not provisioned, and it adds no scaling-path overhead.
- **Scaling Method:** Selects Native Resolution, MAKO Scaler, LS1 Quality, or LS1 Performance. Method changes rebuild only MAKO's private scaler. LS1 falls back to MAKO Scaler for that swapchain when its DLL, translator, resources, format, or pipeline is unavailable; diagnostics record the requested method, active method, and reason. To use scaling, set Steam's Game Resolution to the display's native or highest resolution. On Steam Deck, use 1280 × 800. On Steam Machine, set Maximum Game Resolution to the native resolution of the connected TV or monitor, for example 3840 × 2160 for 4K. Then select a lower in-game resolution, such as 720p or lower. Fewer rendered pixels can improve performance. Use a Scale Factor above 1x to enlarge the image for the display, trading some detail for that performance.
...
- **Experimental Gamescope WSI (Restart):** Adds the guarded MAKO → Gamescope WSI path for FG-only native profiles. Scaling supplies and locks this requirement automatically. It fails closed for missing host manifests or Flatpak runtimes.
...
HDR frame generation and scaling are unavailable in this release. **Disable HDR** remains checked and read-only; MAKO removes inherited `DXVK_HDR` activation. Profiles without Scaling or explicit WSI use the isolated top-role manifest. Scaling uses the exact frame-generation → WSI → spatial-scaling chain even at Native Resolution, followed by the selected MangoHud or vkBasalt role. Flatpak or missing-manifest cases fail closed.
