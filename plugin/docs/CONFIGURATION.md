...
- **Scaling Method:** Selects Native Resolution, MAKO Scaler, LS1 Quality, or LS1 Performance. Method changes rebuild only MAKO's private scaler. LS1 falls back to MAKO Scaler for that swapchain when its DLL, translator, resources, format, or pipeline is unavailable; diagnostics record the requested method, active method, and reason. For the clearest visual comparison, use 2x scaling with a lower source resolution, such as 720p → 1440p or 1080p → 2160p; at 1x or near-native resolution, differences can be subtle.
