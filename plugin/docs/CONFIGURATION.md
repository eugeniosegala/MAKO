...
The experimental **Shaders (Restart)** setting is a per-profile control backed by MAKO's private bundled vkBasalt. It works without a separate installation and never falls back to a system-wide copy. The compact controls cover CAS or DLS sharpening, strength, DLS denoise, optional medium-quality FXAA or SMAA, and a **Shaders** dropdown with Off, Vibrance, Curves, Deband, Technicolor, Sepia, Monochrome, and Vignette. The Default profile automatically uses `~/.config/vkBasalt/vkBasalt.conf`; each saved game or process profile automatically uses its own file under `~/.config/mako-render/vkbasalt/`. Captured Steam games and non-Steam shortcuts use the compact `steam-<app-id>.conf` identity; profiles without a Steam identity use `profile-<short-id>.conf`. The note below the controls shows the exact active path.
...
- **Frame Generation (Restart):** Provisions LSFG device interop, backend ownership, and private generation resources when the game starts. Turn it off for a Scaling-only or Shaders-only profile. When Frame Generation, Scaling, and HDR Brightness Boost are all off, Decky omits the MAKO Renderer layer; a selected shader remains independent.
...
| HDR Brightness Boost and target | Live while MAKO Renderer is loaded; enabling it before launch keeps the Renderer available even when Frame Generation and Scaling are off |
...
| vkBasalt sharpening type, anti-aliasing, **Shaders** selection, sharpness, and DLS denoise while vkBasalt is active | Live; graph changes wait once for vkBasalt's graphics queue, retire the old graph, and may briefly hitch while building the replacement; the saved fallback applies on next launch |
...
Renderer settings are stored in `conf.toml`; profile identity and launcher-only settings use versioned sidecars. Unknown keys are ignored and removed by the next canonical write. Frame Generation provisioning, Scaling fields, **HDR Brightness Boost**, and **Game Swapchain Images** stay in Renderer configuration rather than becoming wrapper environment exports; the wrapper derives process-start Renderer membership from Frame Generation provisioning, Scaling, or HDR Brightness Boost, plus any launcher compatibility settings.
...
**HDR Brightness Boost** under **Advanced Rendering Settings** is intentionally separate from HDR rendering. On a supported HDR display in Gaming Mode, leave HDR enabled in Steam and disable HDR inside the game. The game and MAKO stay SDR, then Gamescope maps the final SDR image through the display's HDR output. Its **Brightness Target** slider ranges from Gamescope's normal 203-nit SDR-on-HDR reference level to a 1,000-nit upper peak target, with 1,000 selected by default when the feature is enabled. This simulates HDR-like brightness only: it does not add HDR colours, highlight detail, inverse tone mapping, or PQ/scRGB content to MAKO's model. The toggle and target apply live while MAKO Renderer is loaded, fail closed if Steam HDR or the active Gamescope display cannot be confirmed, and restore the previous Gamescope brightness when disabled or when the process exits. A 1,000-nit peak may not be sustained at full-screen brightness.

Profiles with Gamescope WSI off use the isolated combined Renderer for provisioned Frame Generation, Scaling, HDR Brightness Boost, or any combination. A Scaling-only or brightness-only profile retains that Renderer but provisions no LSFG interop or backend resources. With WSI on, Scaling uses the managed Renderer → Gamescope WSI → Spatial Scaling order; FG-only profiles omit the lower spatial role. When Frame Generation provisioning, Scaling, and HDR Brightness Boost are all off, Decky disables and removes inherited MAKO Renderer identities; a selected 64-bit MangoHud or vkBasalt layer can still run independently. Missing WSI or spatial-layer dependencies leave WSI disabled and select the combined Renderer when needed, whose surface checks may keep scaling native; an unavailable optional external tool is omitted. Scaling never changes the saved WSI choice. Restart the game after changing Frame Generation provisioning, Scaling, or WSI, and check Live Status for active scaling and the actual source/output sizes.
