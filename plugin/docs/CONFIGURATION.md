...
Press **R1** or tap the small **Hide info** ribbon at the bottom right for a controls-only panel. It hides tutorials, descriptions, tips, warnings, badges, release details, and status panels, leaving settings-section headings, option names, current settings, and action buttons. Press **R1** again or select **Show info** to restore everything. MAKO Decky remembers this display preference on the device across panel openings; it does not change game profiles or individual section-collapse choices. Dialogs retain their instructions and confirmations.
