...
**Advanced Details** shows the installed and bundled MAKO Renderer versions, whether a bundled update is available, host architecture, paths of present Renderer files, and Lossless Scaling detection details. Select any value to copy it. The dialog does not dump generated wrapper or configuration contents.
...
**Enable Shaders (Restart)** is a per-profile control backed by MAKO's private bundled vkBasalt. It works without a separate installation and never falls back to a system-wide copy. The compact controls cover CAS or DLS sharpening, strength, DLS denoise, optional medium-quality FXAA or SMAA, and an **Effects** picker with checkboxes for multiple ordered effects: HDR Look (SDR), Clarity, Levels Plus, Vibrance, Colourfulness, Curves, Deband, Technicolor 2, DPX / Cineon, Bleach Bypass, Noir, Technicolor, Monochrome, Sepia, Film Grain, Vignette, Cartoon, Nostalgia, and Chromatic Aberration. Checking appends an effect to the chain; unchecking and rechecking moves it to the end. Clear all selects no effects. Combining effects increases GPU work. HDR Look changes local contrast to imitate an HDR-style image but remains SDR: it does not enable HDR output, change the display's EOTF, or unlock higher luminance. The Default profile automatically uses `~/.config/vkBasalt/vkBasalt.conf`; each saved game or process profile automatically uses its own file under `~/.config/mako-render/vkbasalt/`. Captured Steam games and non-Steam shortcuts use the compact `steam-<app-id>.conf` identity; profiles without one use `profile-<short-id>.conf`. The note below the controls shows the exact active path.

The Effects picker expands into a compact checklist in Decky's side panel, leaving the game visible while you compare changes. Use Left and Right from an effect or the page control to reach more effects without losing controller focus, then select **Done** to collapse it. Back collapses an open Effects checklist and restores focus to **Choose effects**; pressing Back again returns to Decky's plugin list.

You can edit the displayed file in Desktop Mode or over SSH for any advanced vkBasalt option. On the next MAKO save, Decky merges only the effects and values represented by its compact controls and preserves every other line, comment, and option. Renaming a profile moves its file, deleting that profile deletes the file, and disabling vkBasalt leaves it available for a later re-enable. Enabling or disabling vkBasalt requires a game restart. Sharpening type, anti-aliasing, the selected MAKO effects, sharpness, and DLS denoise apply live while the bundled vkBasalt layer is already active. Selecting a different effect graph waits once for vkBasalt's graphics queue, retires the previous graph to keep GPU memory bounded, and may cause a brief transition hitch while the replacement is built. MAKO's bundled effect sources live under `~/.config/mako-render/vkbasalt/shaders/`; Clarity and Levels Plus are MAKO implementations, not copies of similarly named ReShade shaders. The Qt UI uses the same sidecar, per-profile files, and shader directory, so closing one editor and opening the other preserves the effect order and managed paths. Reopen Qt to pick up changes made in Decky; do not edit the same profile in both at once. Arbitrary manually added ReShade effects and other advanced edits apply on the next launch. If live reload is unavailable, every saved value applies on the next launch. Prepared Flatpak games use the same private bundle and configuration files.
...
- **Real Frame Priority:** Appears only in Fractional Adaptive. Automatic is the default and preserves Fractional Adaptive's existing behavior, including the editable Base FPS Cap. Low, Medium, High, and Very High select 3:2, 2:1, 3:1, and 4:1 target-based real/generated splits, respectively; their option labels recalculate the real-frame cap from Target FPS. The shown FPS and split estimate potential delivery, not a guaranteed game rate or exact cadence. An explicit priority owns the real-frame cap, leaves the saved manual value untouched, and disables Dynamic Cadence Recovery.
...
- **Maximum Adaptive Multiplier:** Appears only in Adaptive mode and selects live `0x` or a 2x–5x ceiling. `0x` pauses synthesis without discarding the saved Adaptive ceiling. Set the ceiling only as high as needed to reach Target FPS; lower ceilings usually preserve quality, while higher ceilings need more headroom.
...
- **Base FPS Cap:** Caps real application frames from Off to 120 FPS in MAKO Decky. Its saved value remains editable but dormant at `0x`; it is unavailable while Steady Base Cap or an explicit Fractional Real Frame Priority owns the cap. Automatic Real Frame Priority preserves the existing manual-cap behavior. Changing the manual cap resets Real Frame Priority to Automatic and disables Dynamic Cadence Recovery.
...
| Fixed Multiplier or Maximum Adaptive Multiplier (`0x`/active), Fixed/Adaptive mode, target, Smooth Cadence, Base FPS Cap, Fractional Real Frame Priority, refresh guard, and recovery | Live when Frame Generation was provisioned at startup |
...
- **Dynamic Cadence Recovery:** For games and emulators that switch native rates, such as 30 FPS gameplay and 60 FPS menus. It periodically probes the real cadence and recalibrates Fixed or Adaptive behavior. Enabling it disables Steady Base Cap and Base FPS Cap, resets Real Frame Priority to Automatic, and in Adaptive mode selects Fractional behavior. The interval ranges from 0.1–3 seconds and defaults to 2 seconds.
