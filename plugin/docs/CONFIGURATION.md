...
Press **R1** or tap the small **Hide info** ribbon at the bottom right for a controls-only panel. It hides tutorials, descriptions, tips, optional warnings, badges, release details, and status panels, leaving settings-section headings, option names, current settings, and action buttons. The Lossless Scaling DLL/model warning, its troubleshooting guidance, and its update action always remain visible. Press **R1** again or select **Show info** to restore other information. The selected control keeps focus as the panel changes height, with its screen position preserved where the scroll range allows. If the focused help button disappears, focus moves to the ribbon. MAKO Decky remembers this display preference on the device across panel openings; it does not change game profiles or individual section-collapse choices. Dialogs retain their instructions and confirmations.

A warning above the controls identifies confirmed LS1 or LSFG availability-check failures for the enabled features, and active LS1 fallback also appears there. **Check for MAKO Decky updates** opens the release page; it does not install anything or change the profile. Apply a MAKO Renderer update when offered and restart the game. An update is a troubleshooting step, not a guarantee: if the warning persists, verify Lossless Scaling and collect diagnostics. A missing DLL instead gets path/installation guidance. Unavailable or older inspectors produce an unknown result and do not trigger a model-failure warning.
