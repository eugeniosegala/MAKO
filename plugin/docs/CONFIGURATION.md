...
Controls are grouped under **Frame Generation**, **Spatial Settings**, **Shaders**, **Performance Settings**, **Advanced Rendering Settings**, compatibility, external tools, and manual overrides. While a game runs, **Live Status** reports the applied Frame Generation and Upscaling state, active model, resolutions, limits, fallbacks, and pending changes. When Frame Generation is enabled, the footer always states that Steam and Decky menus temporarily disable generation; this is a fixed policy notice rather than polled runtime state.
