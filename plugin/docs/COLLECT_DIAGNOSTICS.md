...
Focused presets such as `startup`, `errors`, `adaptive`, `recovery`, `performance`, `layers`, or `hdr` are useful when the maintainer requests a follow-up report. The `adaptive` preset includes contiguous requested-policy pacing aggregates and target-clock state; these are scheduler measurements rather than compositor scanout timestamps. The `recovery` preset includes ordered-acquire quarantine, zero-wait probe and stabilization events, generated-image pressure, pipeline bypasses, history warmup, and render-fence budget misses. Do not substitute a focused preset for the first `all` report unless specifically asked.
