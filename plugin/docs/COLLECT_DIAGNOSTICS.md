...
Focused presets such as `startup`, `errors`, `adaptive`, `recovery`, `performance`, `layers`, or `hdr` are useful when the maintainer requests a follow-up report. The `recovery` preset includes generated-image pressure, pipeline bypasses, history warmup, and render-fence budget misses. Do not substitute a focused preset for the first `all` report unless specifically asked.
