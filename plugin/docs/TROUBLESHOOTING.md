...
HDR frame generation is unavailable in this release. **Disable HDR (Restart)** is checked, read-only, and enforced by the backend. Normal MAKO-managed launches disable HDR exposure, remove inherited `DXVK_HDR` activation, use MAKO's deterministic manifest directory, and disable Gamescope WSI for the game process so MAKO owns the swapchain and a single presentation clock. The Gamescope compositor remains active. **Experimental Gamescope WSI (Restart)** is a per-profile SDR compatibility test for coloured or pixelated motion artifacts; it does not enable HDR, may severely reduce performance, and should be disabled again if generated output or pacing regresses. See the Renderer documentation for [WSI isolation](../../engine/docs/WSI-ISOLATION.md), [optional graphics integrations](../../engine/docs/LAYER-CHAINING.md), and the [HDR pipeline](../../engine/docs/HDR-PIPELINE.md).
