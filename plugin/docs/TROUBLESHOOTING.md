...
HDR frame generation and HDR scaling are unavailable in this release. **Disable HDR** is checked, read-only, and enforced by the backend. MAKO-managed launches disable HDR exposure and remove inherited `DXVK_HDR` activation. Profiles without Scaling Engine or explicit Gamescope WSI use MAKO's isolated top-role manifest directory. A native Steam/Proton profile launched with Scaling Engine enabled instead uses MAKO frame generation → validated Gamescope WSI → MAKO spatial scaling, giving the lower role a variable surface and distinct source/presentation extents while every generated present traverses WSI. Native Resolution keeps that chain provisioned without a model scaler. Changing Scaling Engine always requires a game restart. **Experimental Gamescope WSI (Restart)** exposes the guarded MAKO → WSI path independently for FG-only profiles; it does not enable HDR. An independently selected MangoHud or vkBasalt manifest is staged after the last MAKO role, and no automatic path exposes the host's complete implicit-layer directory. See the Renderer documentation for [WSI isolation](../../engine/docs/WSI-ISOLATION.md), [optional graphics integrations](../../engine/docs/LAYER-CHAINING.md), and the [HDR pipeline](../../engine/docs/HDR-PIPELINE.md).
