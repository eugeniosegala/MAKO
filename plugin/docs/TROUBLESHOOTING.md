...
3. If using Frame Generation or LS1, confirm that the **default public version** of Lossless Scaling is installed through Steam, with beta participation disabled, and check the selected profile's `Lossless.dll` path. Clear an unnecessary **GPU** override and verify **Matched Processes**. Start with Fixed 2x.
