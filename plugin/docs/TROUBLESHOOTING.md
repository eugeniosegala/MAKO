...
HDR frame generation and HDR scaling are unavailable in this release. **Disable HDR** is checked, read-only, and enforced by the backend. MAKO-managed launches disable HDR exposure and remove inherited `DXVK_HDR` activation. Profiles without Scaling Engine or explicit Gamescope WSI use MAKO's isolated manifest directory so the Renderer owns the swapchain and a single presentation clock. A native Steam/Proton profile launched with Scaling Engine enabled instead places only the validated staged Gamescope WSI directory before MAKO, giving the Renderer a variable lower surface and distinct source/presentation extents; the Gamescope compositor remains active in both paths. Native keeps that lane provisioned without running a MAKO scaler. Changing Scaling Engine always requires a game restart. **Experimental Gamescope WSI (Restart)** exposes the same guarded WSI requirement independently for FG-only profiles; it does not enable HDR. An independently selected MangoHud or vkBasalt manifest is staged in its own managed directory after MAKO, and no automatic path exposes the host's complete implicit-layer directory. See the Renderer documentation for [WSI isolation](../../engine/docs/WSI-ISOLATION.md), [optional graphics integrations](../../engine/docs/LAYER-CHAINING.md), and the [HDR pipeline](../../engine/docs/HDR-PIPELINE.md).
